...
const loadUrl = (ambiente.isSviluppo || ambiente.isProduzione) ? "" : "/json_data/rad/bozze/lista.json";
        ...
        const isToFilter = bozza => {
            ...
            return !Object.keys(this.state.filters.values).some(filter => {
                ...
                switch (filter) {
                    ...
                    case "tipo":
                    case "pdt":
                        valoreBozza = bozza[filter].toString();
                        valoreFiltro = this.state.filters.values[filter].toString();
                        isOK = valoreFiltro === "0" || valoreFiltro === valoreBozza;
                        break;
                    case "codice":
                        ...
                        break;
                    case "nominativo":
                        valoreBozza = bozza["nome"].toString().toLowerCase() + " " + bozza["cognome"].toString().toLowerCase();
                    ...
                    <div className="results-list">
                        <DefaultCollapse label="RICERCA AVANZATA" startsOpen={ this.state.collapseOpened }
                            ...
                            <Form className="search-form">
                                <Row>
                                    <Col sm="3">
                                        <DefaultSelect label="Tipo richiesta" onChange={ this.onChange }
                                                       { ...this.state.filters } name="tipo">
                                            {
                                                tipi.map(option => <option key={ option.value }
                                                                           value={ option.value }>{ option.text }</option>)
                                            }
                                        </DefaultSelect>
                                    </Col>
                                    <Col sm="3">
                                        <DefaultInput label="Codice pratica" onChange={ this.onChange } type="text"
                                                      { ...this.state.filters } name="codice"/>
                                    </Col>
                                    <Col sm="6">
                                    ...
                                    <Col sm="3">
                                        <DefaultSelect label="PDT" onChange={ this.onChange }
                                                       { ...this.state.filters } name="pdt">
                                            {
                                                pdt.map(option => <option key={ option.value }
                                                                          value={ option.value }>{ option.text }</option>)
                                            }
                                        </DefaultSelect>
                                    </Col>
