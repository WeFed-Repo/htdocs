...
function chiudi() {
    parent.top.location.href = wrp_script + '/wbOnetoone/chiudi_servizio.jsp';
}
...
function sendRequestxx(_divId, _sendTo, _params, _stateFunction, _waitHtml, xml) {
    ...
    if (_sendTo.length > 0) {
        // Chiamata asincrona
        xml.open('POST', wrp_script + _sendTo, true);
...
function callSwitch(isin, agenzia, deposito, sottodeposito) {
	...
	var hrefValue = cgi_protocol + cgi_host + wrp_script +  '/wbOnetoone' + searchAction + navParam + searchParam + '&mercato=portafoglio&type=switch';
	top.document.location.href = hrefValue;
}
...
function callVendi(isin, agenzia, deposito, sottodeposito) {
	...
	var hrefValue = cgi_protocol + cgi_host + wrp_script +  '/wbOnetoone' + searchAction + navParam + searchParam + '&mercato=portafoglio&type=sell' ;
	top.document.location.href = hrefValue;
}
...
function openLingubot() {
    if ($('#ctntools') === null) {
        window.open(cgi_protocol + cgi_host + wrp_script + '/wbOnetoone/popup/Toolbox.do?tabId=' + tabId + '&OBSKEY=' + obsKey + '&toolOpen=boxtools9', 'toolBox', 'width=215,height=550,status=yes,toolbar=no,menubar=no,resizable=no,scrollbars=yes,top=190,left=750');
    } else {
        boxtools(9)
    }
}
...
function GPS(path, target, param, whoami) {
    ...
    switch (whoami) {
        case "callJSP":
            if (param.indexOf("&") === 0)
                param = param.substring(1, param.length);
            link = wrp_script + path + "?" + param + "&BV_UseBVCookie=Yes&cf=" + Math.random();
            break;
        case "big_pop_JSP":
        ...
        case "callInfoFIN":
            ...
            break;
        case "callInfoFINOBS":
            var calledURL = sole_24h + "/webankinfo/home.jsp%3FBV_UseBVCookie%3DYes%26URL%3D" + cgi_serverurl + wrp_script + "/%26Code%3D" + tibco_code + param;
        ...
        case "callASPADB":
            ...
            link = wrp_script + "/wbOnetoone/ExtUrl.jsp?BV_UseBVCookie=Yes&cf=" + Math.random() + "&calledURL=" + calledURLL + param;
            if (target === "yes") {
            ...
            break;
        case "callPFV_XP":
            var calledURL = wrp_script + path + "%3FBV_UseBVCookie%3DYes" + param;
            link = wrp_script + "/wbOnetoone/ExtUrl.jsp?BV_UseBVCookie=Yes&cf=" + Math.random() + "&calledURL=" + calledURL;
            target = "_blank";
...
function tinyPopup(_action, _params, _title) {
    url = wrp_script + "/wbOnetoone/cstPopup" + _action + "?" + _params;
...
function trading_desk(path, param) {
    ...
    var sizeW = 640;
    var sizeW = 480;
    var link = wrp_script + path + "?BV_UseBVCookie=Yes&cf=" + Math.random() + param;
...
function openSendEmail(divId) { // divId=divtoupdate
    $('#' + divId).load(wrp_script + '/wbOnetoone/wbblack/ajax/json/getCoordBancarie.jsp', { divId: divId, idx: $('#selectConti').val() },
        function() {
            openAlert(divId);
        }
    );
...
function openSendEmailSimple(divId) { // divId=divtoupdate
    $('#' + divId).load(
        wrp_script + '/wbOnetoone/wbblack/ajax/json/getCoordBancarieSimple.jsp', {
            divId: divId,
            idx: $('#selectConti').val()
        },
        function() {
...
function getLinkRadiocor(codRadiocor) {
    var url = sole_24h + '?URL=' + escape(cgi_serverurl + wrp_script + '/');
...
function msApriFondo(pagina, bloccoparametri) {
    pagetocall = top.location.protocol + "//" + top.location.host + wrp_script + "/wbOnetoone/mf/servlet/" + pagina + ".do?" + bloccoparametri + "&BSCNT=FNZ_NAV&tabId=nav_priv_wbx_investimenti&OBSKEY=nav_priv_wbx_vantaggi_fs&OBSKEY3=nav_priv_wbx_scheda_fondo";
    top.location.href = pagetocall;
}

function msVediTutti() {
    pagetocall = top.location.protocol + "//" + top.location.host + wrp_script + "/wbOnetoone/mf/servlet/fundquickrank/default.aspx.do?sortby=ReturnM36&sortorder=desc&tab=Performance&OBSCNT=FNZ_NAV&tabId=nav_priv_wbx_investimenti&OBSKEY=nav_priv_wbx_classifiche";
    top.location.href = pagetocall;
}
...
function getCarrello(callback) {
    jqAJAXCall('get', 'json', wrp_script + '/FMP/returnCart.jsp', function(json) { getCarrelloOK(json, callback); }, function(jqXHR, textStatus, errorThrown) { getCarrelloKO(jqXHR, textStatus, errorThrown, callback); }, { type: 'fida', cf: Math.random() });
}

function checkCarrello(cart, succFunc, failFunc) {
    jqAJAXCall('post', 'json', wrp_script + '/FMP/returnCart.jsp', succFunc, failFunc, { type: 'check', cart: JSON.stringify(cart) });
}

function overwriteCarrello(cart, callback) {
    jqAJAXCall('post', 'json', wrp_script + '/FMP/checkCart.do', function(json) { overwriteCarrelloOK(json, callback); }, function(jqXHR, textStatus, errorThrown) { overwriteCarrelloKO(jqXHR, textStatus, errorThrown, callback); }, { cart: JSON.stringify(cart) });
}
...
function delFondo(el, isin) {
    ...
    jqAJAXCall('post', 'json', wrp_script + '/FMP/checkCart.do', function(json) { delFondoOK(json, riga); }, delFondoKO, { deleteFund: isin });
}

function delCarrello() {
    jqAJAXCall('post', 'json', wrp_script + '/FMP/checkCart.do', delCarrelloOK, delCarrelloKO, { deleteCart: 'OK' });
}
...
function changeParamCarrello(ordColumn) {
    ...
    jqAJAXCall('post', 'json', wrp_script + '/FMP/changeParameterCart.jsp', null, null, { "ordColumn": ordColumnCart, "ordDir": ordDirCart, "quantita": importo });
}
...
function checkCarrelloOK(json, cart, callback) {
    if (json.alert) {
        ...
        arrRightBtn.eq(0).attr('title', 'vedi carrello "' + profiloOld + '"')
            .attr('href', wrp_script + '/wbOnetoone/2l/do/FMP/viewCart.do?OBSCNT=FNZ_NAV&tabId=nav_priv_wbx_investimenti&OBSKEY=nav_priv_wbx_carrello');
...
function callExt(action, param) {
    var hrefValue = cgi_protocol + cgi_host + wrp_script + action + param + "&cf=" + Math.random();
    top.document.location.href = hrefValue;
}
...
function getFogliInfo(fondo) {
    jqAJAXCall('get', 'html', wrp_script + '/FMP/fogliInformativi.jsp', getFogliInfoOK, getFogliInfoKO, { societaGestione: fondo, cf: Math.random() });
}
...
function openPopAucap(aucapUrl) {
    if (!wrp_script) wrp_script = "";
    window.open(wrp_script + aucapUrl, 'aucap', 'width=463,height=610,top=0,left=0,status=yes,toolbar=no,menubar=no,resizable=yes,scrollbars=yes');
}
...
function emailoGet() {
    ...
    if (emailoQuest !== "" && emailoQuest !== "Scrivi un messaggio..." && emailoQuest.split(" ").length >= 3 && emailoQuest !== emailoLastQuest) {
        ...
        jqAJAXCall('get', 'json', wrp_script + "/wbOnetoone/wbblack/commons/virtass.jsp",
...
function fpFinAnimationShow() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $('#scopriFinanziamento').attr('href', wrp_script + "/wbOnetoone/3l/action/wbOnetoone/schedaPrivata.action?KEYLAND=Catalogo_acquisto&OBSCNT=TAB&tabId=nav_priv_wbx_finanziamenti&OBSKEY=nav_priv_wbx_mutuo_webank&OBSKEY3=nav_priv_wbx_cat_acquisto");
            $('#faiPreventivoFinanziamento').attr('href', wrp_script + "/wbOnetoone/2l/do/wbOnetoone/schedaProdotto.do?OBSCNT=FNZ_NAV&tabId=nav_priv_wbx_finanziamenti&OBSKEY=nav_priv_wbx_fai_il_preventivo");
            $('#scopriPrestito').attr('href', wrp_script + "/wbOnetoone/2l/action/wbOnetoone/WsRichiestaCatalogo.action?OBSCNT=FNZ_NAV&tabId=nav_priv_wbx_finanziamenti&OBSKEY=nav_priv_wbx_prestito_x_te_wb&sezione=prestiti");
