/* jshint strict: false */
/* global $, AmCharts, cgi_script, escape, jqAJAXCall, printLog, lowerZIndex, riseZIndex,
	setOverOption, unSetOverOption, updateStar, focusOptionDownRapp, focusOptionUpRapp, jqc,
	setLoadingOnObject, unsetLoadingOnObject, moveToInteger, format2DecNum, setOverSelectRapp, unSetOverSelectRapp	*/
...
var oroDelCapitano = (function () {
    ...
    var librerie, pathLibrerie, forceJS,
        debugOroCapitano = true,
        ...
        consultareEl, ombraSubGrafBottom, noChartTitle, jsChartPrintClass,
        sceltaRapportoFP, sceltaRapportoValFP, parameters, conto, closeOnBlur2, hiddenIds2,
        // Metodi
        sceltaRapportoFuncFP, removeBoxTesto, removeBoxTestoSuccess,
    ...
    init = function () {
        if (typeof window.librerie === 'undefined') {
            librerie = false;
        } else {
            librerie = window.librerie;
        }
        if (typeof window.pathLibrerie === 'undefined') {
            pathLibrerie = '';
        } else {
            pathLibrerie = window.pathLibrerie;
        }
        if (typeof window.forceJS === 'undefined') {
            forceJS = false;
        } else {
            forceJS = window.forceJS;
        }
        ...
        consultareEl = $('#consultare');
        ombraSubGrafBottom = $('#ombraSubGrafBottom');

        consultare = !(!consultareEl.length || consultareEl.val().toLowerCase() !== 'true');
    ...
    sceltaRapportoFuncFP = function (option) {
        ...
        var linkExportXls = $('#exportXls'),
            urlExportXls = "/webankpri/pktrading/posizioneFinanziaria.do?format=xls&accounts=" + encodeURIComponent(option.value),
            parameters = {
    ...
    removeBoxTesto = function () {
        var url = '/situazionePatrimonialeNote.jsp',
            AjaxUrl = (!librerie) ? cgi_script + '/pktrading' + url : pathLibrerie + url.replace(/\.jsp(\?)?/i, '.php$1');
        jqAJAXCall('post', 'json', AjaxUrl, removeBoxTestoSuccess, null, null);
    };
    ...
    initStaticLinks = function () {
        ...
        if (hasCT) {
            $('#boxContoDeposito').find(' a').attr('href', cgi_script + '/wbOnetoone/ctfpBanking.do?OBSCNT=FNZ_NAV&tabId=nav_priv_wbx_sportello_ct&OBSKEY=nav_priv_wbx_sportello_ct&cf=' + cf);
        } else {
            $('#boxContoDeposito').find('a').attr('href', cgi_script + '/wbOnetoone/2l/do/banking/depositAccounts/WsDepositAccountsMigrationInput.do?OBSCNT=FNZ_NAV&tabId=nav_priv_wbx_myhome&OBSKEY=nav_priv_wbx_passa_al_nuovo_conto&cf=' + cf);
        }
        if (consultare) {
            $('#boxPortafoglio').find('a').attr('href', cgi_script + '/wbOnetoone/2l/do/wbOnetoone/schedaProdotto.do?OBSCNT=TAB&tabId=nav_priv_wbx_trading_wt&OBSKEY=nav_priv_wbx_dep_titoli_wt&OBSKEY3=nav_wbx_priv_scopri_il_deposito_wt&cf=' + cf);
        } else {
            $('#boxPortafoglio').find('a').attr('href', cgi_script + '/wbOnetoone/2l/do/wbOnetoone/schedaProdotto.do?OBSCNT=TAB&tabId=nav_priv_wbx_trading_wt&OBSKEY=nav_priv_wbx_dep_titoli_wt&OBSKEY3=nav_wbx_priv_scopri_il_deposito_wt&cf=' + cf);
        }
    };
    ...
    initDinamicLinks = function () {
        ...
        if (!hasCT) {
            ...
            if (selAccounts.length > 1) {
                linkDett1 = cgi_script + '/wbOnetoone/2l/do/banking/WsAccountsList.do?OBSCNT=FNZ_NAV&tabId=nav_priv_wbx_sportello&OBSKEY=nav_priv_wbx_saldo&cf=' + cf;
                linkDett2 = linkDett1;
                linkDett3 = linkDett1;
            } else {
                linkDett1 = cgi_script + '/wbOnetoone/2l/do/banking/WsAvailableBalanceMultiAction.do?tabId=nav_priv_wbx_sportello&OBSKEY=nav_priv_wbx_saldo&OBSKEY3=&OBSCNT=&cf=' + cf + '&CC=' + encodeURIComponent(selAccounts[0]);
                linkDett2 = cgi_script + '/wbOnetoone/2l/do/banking/WsAvailableBalanceCD.do?tabId=nav_priv_wbx_sportello&OBSKEY=nav_priv_wbx_saldo&OBSKEY3=&OBSCNT=&cf=' + cf + '&accountId=' + encodeURIComponent($('#contodep').val());
                linkDett3 = cgi_script + '/wbOnetoone/2l/do/banking/WsAvailableBalanceDER.do?tabId=nav_priv_wbx_sportello&OBSKEY=nav_priv_wbx_saldo&OBSKEY3=&OBSCNT=&cf=' + cf + '&DER=' + encodeURIComponent($('#contoder').val());
            }
        } else {
            // Link CC, CD e DER con CT
            if (selAccounts.length > 1) {
                linkDett1 = cgi_script + '/wbOnetoone/2l/do/banking/WsAccountsListTechnical.do?OBSCNT=FNZ_NAV&tabId=nav_priv_wbx_trading_wt&OBSKEY=nav_priv_wbx_saldo_liquidita_wt&cf=' + cf;
                linkDett2 = linkDett1;
                linkDett3 = linkDett1;
            } else {
                linkDett1 = cgi_script + '/wbOnetoone/2l/do/banking/WsAvailableBalanceMultiAction.do?tabId=nav_priv_wbx_trading_wt&OBSKEY=nav_priv_wbx_saldo_liquidita_wt&OBSKEY3=&OBSCNT=&cf=' + cf + '&CC=' + encodeURIComponent(selAccounts[0]);
                linkDett2 = cgi_script + '/wbOnetoone/2l/do/banking/WsAvailableBalanceCD.do?tabId=nav_priv_wbx_trading_wt&OBSKEY=nav_priv_wbx_saldo_liquidita_wt&OBSKEY3=&OBSCNT=&cf=' + cf + '&accountId=' + encodeURIComponent($('#contodep').val());
                linkDett3 = cgi_script + '/wbOnetoone/2l/do/banking/WsAvailableBalanceDER.do?tabId=nav_priv_wbx_trading_wt&OBSKEY=nav_priv_wbx_saldo_liquidita_wt&OBSKEY3=&OBSCNT=&cf=' + cf + '&DER=' + encodeURIComponent($('#contoder').val());
            }
        }
        ...
        $('#valLegenda3').attr('data-href', cgi_script + '/wbOnetoone/2l/do/bnkg/listaVincoliConti.do?OBSCNT=FNZ_NAV&tabId=nav_priv_wbx_sportello&OBSKEY=nav_priv_wbx_riepilogo_le&account=' + encodeURIComponent(account) + '&cf=' + cf);
        $('#valLegenda5').attr('data-href', cgi_script + '/wbOnetoone/2l/do/pktrading/portafoglioReale.do?OBSCNT=TAB&tabId=nav_priv_wbx_trading_wt&OBSKEY=nav_priv_wbx_tit_in_portaf_wt&OBSKEY3=portafoglio_reale&account=' + encodeURIComponent(account) + '&cf=' + cf);
        $('#valLegenda6').attr('data-href', cgi_script + '/wbOnetoone/2l/jsp/pktrading/investimenti/portafoglioInvestimenti.do?OBSCNT=FNZ_NAV&tabId=nav_priv_wbx_investimenti&OBSKEY=nav_priv_wbx_strum_in_portaf&tipo=fondi&account=' + encodeURIComponent(account) + '&cf=' + cf);
        $('#valLegenda7').attr('data-href', cgi_script + '/wbOnetoone/2l/jsp/pktrading/investimenti/portafoglioInvestimenti.do?OBSCNT=FNZ_NAV&tabId=nav_priv_wbx_investimenti&OBSKEY=nav_priv_wbx_strum_in_portaf&tipo=pct&account=' + encodeURIComponent(account) + '&cf=' + cf);
    };
    ...
    initChart = function () {
        ...
        if (labelActiveEls.length > 0) {
            ...
            // WRITE
            jsChart[chartPatrimonio].write(chartPatrimonio);
    ...
    initSubChart = function (num) {
        ...
        if (subLabelActiveEls && subLabelActiveEls.length > 0) {
            ...
            balloon.borderAlpha = 0;
            // WRITE
            jsChart[subChartPatrimonio].write(subChartPatrimonio);
            $('#coveringDiv2').removeClass('empty').hide();
    ...
    subLegendaLVLoad = function (durationTime) {
        ...
        if (jsonData['3'] === null) {
            var url = '/listaLVinc.do',
                AjaxUrl = (!librerie) ? cgi_script + '/bnkg' + url : pathLibrerie + url.replace(/\.do(\?)?/i, '.php$1'),
                parameters = {
                    "method": "group",
    ...
    findElNum = function (panelDefault) {
        var elId = panelDefault.find('.labelLegenda:first').attr('id');
    ...
    jsRollOverSlice = function (event) {
        //if (debugOroCapitano) { printLog('jsRollOverSlice'); }
        var dataItem = event.dataItem;
        // if (typeof console !== 'undefined') { console.log('doSliceOver: ' + dataItem.index); }
    ...
    jsRollOutSlice = function () {
        //if (debugOroCapitano) { printLog('jsRollOutSlice'); }
        // var dataItem = event.dataItem;
        // if (typeof console !== 'undefined') { console.log('doSliceOut: ' + dataItem.index); }
    ...
    loadChartData = function (parameters) {
        var url = '/posizioneFinanziaria.do',
            AjaxUrl = (!librerie) ? cgi_script + '/pktrading' + url : pathLibrerie + url.replace(/\.do(\?)?/i, '.php$1');
        setLoadingOnObject('grafPatrimonio');
        jqAJAXCall('post', 'json', AjaxUrl, parseChartData, null, parameters);
        initDinamicLinks();
    };
    ...
    parseChartData = function (data) {
        ...
        var totale = moveToInteger(data.posizioneFinanziaria.totale),
            ...
            depositoGestito, pct;
        $('#liquiditaTotale').html(format2DecNum(moveToInteger(liquidita.totale)));
        $('#legendaTotale').html(format2DecNum(totale));
        $('#titTotale').html(format2DecNum(totale));
        expandableLabel['1'] = false;
        // --- LIQUIDITA ---
        // Saldo CC
        $('#subLegenda1').collapse('hide');
        if (contoCorrenteTot !== 0) {
            $('#numLegenda1').html(format2DecNum(contoCorrenteTot)).closest('.panel-default').addClass('active');
            ...
            for (indCC = 1; indCC <= 6; indCC += 1) {
                ...
                if (valore !== 0) {
                    $('#numLegenda1_' + indCC).html(format2DecNum(valore)).closest('.saldoRow').removeClass('hidden').addClass('active');
                } else {
                    $('#numLegenda1_' + indCC).html(format2DecNum(0)).closest('.saldoRow').removeClass('active').addClass('hidden');
                }
                jsonData['1'][indCC.toString()].value = valore;
            }
        } else {
            $('#numLegenda1').html(format2DecNum(contoCorrenteTot)).closest('.panel-default').removeClass('active');
            jsonData['0']['1'].value = contoCorrenteTot;
        }
        // Saldo CD e LV
        ...
        if (liquidita.contoDeposito) {
            ...
            if (contoDeposito > 0) {
                $('#numLegenda2').html(format2DecNum(contoDeposito)).closest('.panel-default').removeClass('hidden').addClass('active');
                jsonData['0']['2'].value = contoDeposito;
            } else {
                $('#numLegenda2').html(format2DecNum(contoDeposito)).closest('.panel-default').removeClass('hidden').removeClass('active');
                jsonData['0']['2'].value = contoDeposito;
            }
            expandableLabel['3'] = (lineeVincolate > 0);
            if (lineeVincolate > 0 || consultare) {
                $('#numLegenda3').html(format2DecNum(lineeVincolate)).closest('.panel-default').removeClass('hidden').addClass('active');
                $('#labelLegenda3').html('Saldo linee vincolate').attr('href', '#!');
                jsonData['0']['3'].value = lineeVincolate;
            } else {
                $('#numLegenda3').html(format2DecNum(lineeVincolate)).closest('.panel-default').removeClass('hidden').removeClass('active');
                $('#labelLegenda3').html('Per un tasso pi&ugrave; alto apri una linea vincolata!').attr('href', cgi_script + '/wbOnetoone/2l/do/bnkg/vincoloConti.do?OBSCNT=FNZ_NAV&tabId=nav_priv_wbx_investimenti&OBSKEY=nav_priv_wbx_apri_nl&cf=' + Math.random());
                $('#valLegenda3').attr('data-href', cgi_script + '/wbOnetoone/2l/do/bnkg/vincoloConti.do?OBSCNT=FNZ_NAV&tabId=nav_priv_wbx_investimenti&OBSKEY=nav_priv_wbx_apri_nl&cf=' + Math.random());
                jsonData['0']['3'].value = lineeVincolate;
            }
        } else {
        ...
        if (liquidita.contoDerivati) {
            ...
            if (contoDerivatiTot !== 0) {
                $('#numLegenda4').html(format2DecNum(contoDerivatiTot)).closest('.panel-default').removeClass('hidden').addClass('active');
                ...
                for (indDER = 1; indDER <= 2; indDER += 1) {
                    ...
                    if (valore !== 0) {
                        $('#numLegenda4_' + indDER).html(format2DecNum(valore)).closest('.saldoRow').addClass('active');
                    } else {
                        $('#numLegenda4_' + indDER).html(format2DecNum(valore)).closest('.saldoRow').removeClass('active');
                    }
                    jsonData['4'][indDER.toString()].value = valore;
                }
            } else {
                $('#numLegenda4').html(format2DecNum(contoDerivatiTot)).closest('.panel-default').removeClass('hidden').removeClass('active');
                jsonData['0']['4'].value = contoDerivatiTot;
            }
        } else {
        ...
        if (data.posizioneFinanziaria.portafoglio) {
            ...
            $('#portafoglioTotale').html(format2DecNum(moveToInteger(portafoglio.totale)));

            // Portafoglio titoli
            if (depositoAmministratoTot !== 0) {
                $('#numLegenda5').html(format2DecNum(depositoAmministratoTot)).closest('.panel-default').removeClass('hidden').addClass('active');
                ...
                for (indDA = 1; indDA <= 7; indDA += 1) {
                    ...
                    if (valore > 0) {
                        $('#numLegenda5_' + indDA).html(format2DecNum(valore)).closest('.saldoRow').removeClass('hidden').addClass('active');
                    } else {
                        $('#numLegenda5_' + indDA).html(format2DecNum(valore)).closest('.saldoRow').addClass('hidden').removeClass('active');
                    }
                    jsonData['5'][indDA.toString()].value = valore;
                }
            } else {
                $('#numLegenda5').html(format2DecNum(depositoAmministratoTot)).closest('.panel-default').removeClass('hidden').removeClass('active');
                jsonData['0']['5'].value = depositoAmministratoTot;
            }
            ...
            if (depositoGestito > 0) {
                $('#numLegenda6').html(format2DecNum(depositoGestito)).closest('.panel-default').removeClass('hidden').addClass('active');
                $('#labelLegenda6').html('Portafoglio fondi e sicav').attr('data-href', '#!');
                jsonData['0']['6'].value = depositoGestito;
            } else {
                $('#numLegenda6').html(format2DecNum(depositoGestito)).closest('.panel-default').removeClass('hidden').removeClass('active');
            ...
            if (portafoglio.PCT) {
                pct = moveToInteger(portafoglio.PCT);
                if (pct > 0) {
                    $('#numLegenda7').html(format2DecNum(pct)).closest('.panel-default').removeClass('hidden').addClass('active');
                    jsonData['0']['7'].value = pct;
                } else {
                    $('#numLegenda7').html(format2DecNum(pct)).closest('.panel-default').removeClass('hidden').removeClass('active');
                    jsonData['0']['7'].value = pct;
                }
            } else {
        ...
        unsetLoadingOnObject('grafPatrimonio');
        if (debugOroCapitano) {
    ...
    parseLVData = function (data, durationTime) {
        ...
        $.each(data.lineeVincolate, function (key, value) {
            ...
            subLegendaLV.append('<div class="saldoRow ' + classNames + '">' +
                ...
                '<div id="valLegenda3_' + num + '" class="saldoVal valLegenda"><span id="numLegenda3_' + num + '">' + format2DecNum(moveToInteger(value)) + '</span>&nbsp;&euro;</div>' +
                '</div>');
        });
        subLegendaOpen("3", true, durationTime);
    };
    ...
    readData = function (num, type) {
        ...
        if (forcePerc) {
            limit1perc = parseInt((dataTot * percUnit) / 100, 10);
        ...
        $.each(dataHash, function (key, value) {
            ...
            if (!panelDefault.hasClass('disabled') && value.value !== null) {
                ...
                perc = format2DecNum(moveToInteger((val * 100) / dataTot));
                elNum = findElNum(panelDefault);
                ...
                if (val > 0) {
                    ...
                    if (type === 'js') {
                        outputData.push({
                            "title": value.title,
                            "description": perc + '% (' + format2DecNum(value.value) + ' EUR)' + dettTip,
                            "value": dataArr[index].toFixed(2)
                        });
                    } else {
                        outputData += '<slice title=\'' + value.title + '\' description=\'' + perc + '% (' + format2DecNum(value.value) + ' EUR)' + dettTip + '\'>' + dataArr[index].toFixed(2) + '</slice>';
                    }
                }
                if (calcolaTot) {
                    tot += val;
        ...
        if (intNum === 0) {
            $('#liquiditaTotale').html(format2DecNum(totLiquidita));
            $('#portafoglioTotale').html(format2DecNum(totPortafoglio));
            $('#legendaTotale').html(format2DecNum(tot));
            $('#titTotale').html(format2DecNum(tot));
        } else if (intNum === 5) {
            jsonData[0][num].value = tot;

            $('#numLegenda' + num).html(format2DecNum(tot));
        }
    ...
    createSelectRapp2 = function (idSelect, idHidden, funcSelect) {
        ...
        var selectRapp = $(jqc(idSelect)), selector, inputOption, selectorOptions;
