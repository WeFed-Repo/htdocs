...
function mGetData()
{
	...
	$.ajax({
		...
		dataType: "json",
		success: function(data) {
			$("#calcolatoremutui").removeClass("loading");
			showResults();
			// stopPulseCalc();
			mRes.html("");
			mutui = data.mutui;
			...
			$.each(mutui,function(i,v){
				...
				if (mutuo.disponibile)
				{
					...
					mGauge = $("<div>").addClass("rata-value").append($("<span>").addClass("rata-text").html(formatMigliaia(mutuo.rata.toString().split(".")[0]) + ","+ mutuo.rata.toString().split(".")[1]+"&nbsp;&euro;<span class=\"smaller\">&nbsp;al mese</span>"));
					mGauge.attr("w",mrata);
					...
					*/
					...
					if (isDetail)
					{
						...
						mNodeDocUl.append($("<li>").append($("<a>").addClass("btn-icon").html("<i class='icon icon-file_pdf icon-documenti'></i><span>Scheda prodotto e Informazioni Generali</span>").attr({"href": mutuo.link, "target": "_blank"})));
		...
		success: function(data) {
			...
			$.each(mutui,function(i,v){
				...
				if (mutuo.disponibile)
				{
					...
					if (!isDetail) mNode.append(
							$("<a>").addClass("btn-icon documenti").html("<i class=\"icon icon icon icon-file_pdf\"></i><span>Scheda prodotto e Informazioni Generali</span>").attr({"href": mutuo.link, "target": "_blank"})
					);
					...
					mGauge = $("<div>").addClass("rata-value").append($("<span>").addClass("rata-text").html(formatMigliaia(mutuo.rata.toString().split(".")[0]) + ",<span class=\"small\">"+ mutuo.rata.toString().split(".")[1]+"&nbsp;</span>&euro;<span class=\"smaller\">&nbsp;al mese</span>"));
					mGauge.attr("w",mrata);
					...
					*/
					...
					if (isDetail)
					{
						// Dettagli documenti ed assicurazioni
						mNodeDocUl = $("<ul>").addClass("documenti");
						mNodeDocUl.append($("<li>").append($("<a>").addClass("btn-icon").html("<i class='icon icon-file_pdf'></i><span>Scheda prodotto e Informazioni Generali</span>").attr({"href": mutuo.link, "target": "_blank"})));
						mNodeDocUl.append($("<li>").append($("<a>").addClass("btn-icon").html("<i class='icon icon-file_pdf'></i><span>Piano di ammortamento</span>").attr({"href": mutuo.pianoammortamento, "target": "_blank"})));
