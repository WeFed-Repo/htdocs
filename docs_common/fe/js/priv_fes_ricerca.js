...
var fesSearchDataUrl = "", fesConfUrl = "";
...
if (!is4s)
{
	fesSearchDataUrl = (!isLib) ? cgi_script + "/FMP/fesTabella.do"  : "/wscmn/xml/fes_tabella.php";
...
fesTableCols = {
	...
	"fesSharpe": {"nome": "Sharpe", "classe":"rischio"},
					...
					$.each(data.data,function(k,v)
					{
						...
						fesCb.click(function(e){
							...
							if($(this).is(":checked"))
							{
								if(fesConfSelected.length>=fesConfMax)
								{
									$(this).attr("data-tooltip","<strong>Fondo aggiunto al confronto.</strong><br>&Egrave; possibile confrontare al massimo 2 fondi alla volta.<br><br><a href=\"javascript:;\" onclick=\"fesTabConf.trigger('click')\">Vai alla pagina \"confronta\"</a>");
								}
								else
								{
						...
						if (!is4s)
						{
							...
							/* Se e' il porting icona operativa che carica overlay */
							if (isPorting)
							{
								fesTrOpeIcoA.addClass("btn-icon inited").attr("data-fundname",v.nome)
								.append($("<i>").addClass("icon icon-2x icon-ico_azioni02A"));
								fesTrOpeIcoA.click(fesIcoOpeOverlay);
							...
							else
							{
								...
								fesTrOpeMenu = $("<div>").addClass("sezioneLinks2").css({"margin-left": "-113px", "margin-top": "-21px"}).hide();
								fesTrOpeIco = $("<div>").addClass("linkTooltip").append(fesTrOpeIcoA, fesTrOpeMenu);
...
function fesSelect(btnSel)
{
	...
	else
	{
		...
		fesSliderRefresh = $("<a>").addClass("refresh");
	...
	if (fesCurrentField == "fesCategoria")
	{
		...
		fesCatSelectConsole  = $("<div>").addClass("btnConsole");
		fesCatSelectBtnSel  = $("<a>").addClass("btn").attr("id","btnSeleziona").html("seleziona tutti");
		fesCatSelectBtnDes  = $("<a>").addClass("btn").attr("id","btnDeseleziona").html("annulla selezione");
...
function fesTabInit()
{
		...
		fesConfButtonClose.click(function()
		{
			...
			if ($("#tabConfronta").hasClass("active"))
			{
				fesConfShow();
			}
...
function fesRicShow()
{
	...
	if(!fesRicStarted)
		...
		fesSelOl = $("<div>").attr("id","fesSelector").addClass("selOverlay");
		fesSelConfirm = $("<a>").addClass("aButton").attr("href","javascript:;");
		...
		$.each($(".btnMultiSel"), function(i,v)
		{
			...
			var fDivReset = $("<a>").addClass("reset").html("&nbsp;").attr({"href":"javascript:;","title":"Resetta criterio di ricerca"}).hide();
		...
		// Trova l'ultimo elemento nel menù laterale
		...
		$("#fesSearchTableSelect").on("change", function(){
			...
			if ($(this).val() == "perso"){
				...
				$.each(fesTablePersCols,function(k,v)
				{
					...
					tPcol = $("<label>").addClass("selEl");
					tPcolInput = $("<input>").attr({"type":"checkbox","name":"persCol","value":k});
...
{
	if (!is4s) {
		if($("#fesRicWrap").length) {
			$.ajax({
				url: "/wscmn/html/fes_notelegali.html?rand=" + Math.random(),
				dataType: "html",
				success: function(data)
					{
						var divNote = $("<div>").css({"padding":"30px 0 0 163px","clear":"both"}).attr("id","noteLegaliFondo");
						divNote.append($("<div>").addClass("filettoGenericoOrizzontale02"));
						divNote.append("<br>" + data)
						$("#contenuti").after(divNote);
					}
			});
		}
	}
	isPorting = $("#contenuti").hasClass("porting");
}
);