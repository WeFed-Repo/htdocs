...
var movimentiFunction = (function() {
    ...
    loadGrafico = function(type) {
        ...
        if (typeof chartMovDataArray[meseMovDiff] !== 'undefined') { doUpdateCharts(chartMovDataArray[meseMovDiff]); } else {
            ...
            if (type === 'next') { parameters.balance = lastSaldo; } else if (type === 'prev') { parameters.balance = firstSaldo; }
    ...
    createChartMovimenti = function() {
        ...
        if (isTouchDevice() && chartMov.chartCursor.addListener) {
            ...
            chartMov.chartCursor.addListener('changed', function (e) { setValueOntable(e); });
        }
    };
    ...
    doUpdateCharts = function(data) {
        ...
        if (meseMovDiff === -5) { slidelayerleft.hide(); } else { slidelayerleft.attr('title', 'Vai al mese di ' + todayPrevMonth.format('mmmm yyyy').toLowerCase()); }
        if (meseMovDiff === 0) { slidelayerright.hide(); } else { slidelayerright.attr('title', 'Vai al mese di ' + todayNextMonth.format('mmmm yyyy').toLowerCase()); }
        ...
        if (data.movMese.catEnt.length > 0) {
            ...
            $.each(causaliEnt, function(key, value) {
                ...
                listeCausali.eq(0).append($('<li>').append($('<span>', { "class": "label" }).html(label + '&nbsp;<span>' + valueStr + '&nbsp;&euro;</span>').prepend($('<span>', { style: "background-color:" + key, class: "legendColor" }))));
            });
        }
        ...
        if (data.movMese.catUsc.length > 0) {
            ...
            $.each(causaliUsc, function(key, value) {
                ...
                listeCausali.eq(1).append($('<li>').append($('<span>', { "class": "label" }).html(label + '&nbsp;<span>' + valueStr + '&nbsp;&euro;</span>').prepend($('<span>', { style: "background-color:" + key, class: "legendColor" }))));
            });
        }
        ...
        if (totDiff === 0) {
            ...
            if (totDiff > 0) { mdlcenter.removeClass('down').addClass('up'); } else { mdlcenter.removeClass('up').addClass('down'); }
            ...
            item.mouseover(function(e) {
                ...
                if (scrollTopCausali[index] < 0) {
                    ...
                    scrollCausali = function() {
                        ...
                        scrollTimeout = null;
                        if (scrollTopCausali[index] === 0) { el.css({ visibility: 'hidden' }); } else { scrollTimeout = setTimeout(scrollCausali, scrollInterval); }
                    };
            ...
            item.mouseover(function(e) {
                ...
                if (scrollTopCausali[index] > diff) {
                    ...
                    scrollCausali = function() {
                        ...
                        scrollTimeout = null;
                        if (scrollTopCausali[index] === diff) { el.css({ visibility: 'hidden' }); } else { scrollTimeout = setTimeout(scrollCausali, scrollInterval); }
                    };
                    if (scrollTimeout === null) { scrollTimeout = setTimeout(scrollCausali, scrollInterval); }
                }
            }).mouseout(function() {
