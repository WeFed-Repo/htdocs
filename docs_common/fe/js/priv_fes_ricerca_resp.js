...
if (!is4s)
{
	fesSearchDataUrl = (!isLib) ? cgi_script + "/investimenti/FesTabellaJson.action?cf=" + Math.random()   : "/wscmn/xml/fes_tabella.php";
//	fesConfUrl = (!isLib) ? cgi_script + "/FMP/compareFunds.do"  : "/librerie/include/commons/ajax/fes_confronta_resp.php";	//modifica 1 : TODO OBSKEY
	fesConfUrl = (!isLib) ? cgi_script + "/investimenti/ConfrontaFondi.action?OBSCNT=FNZ_NAV&tabId=nav_priv_wbx_investimenti&OBSKEY=nav_priv_wbx_vendita&cf=" + Math.random()  : "/librerie/include/commons/ajax/fes_confronta_resp.php";
}
else
{
	fesSearchDataUrl = (!isLib) ? bpmDispatcher : "/wscmn/xml/fes_tabella.php";
	fesConfUrl = (!isLib) ? bpmDispatcher + "compareFunds4s"  : "/librerie/include/commons/ajaxfes_confronta_resp.php";
}
...
var fesSchedaFondoUrl = (!isLib) ? cgi_script + "/wbOnetoone/2l/action/investimenti/SchedaFondo.action?OBSCNT=FNZ_NAV&tabId=nav_priv_wbx_investimenti&OBSKEY=nav_priv_wbx_vendita&cf=" + Math.random()  : "/connect.php?page=strutt_priv.php&tpl=tpl_responsive_inv_scheda_fondo.php&responsive=y&liv0=4&liv1=2&liv2=6&liv3=";
