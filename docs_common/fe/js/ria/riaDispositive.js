...
var disponiTrasferimentoCD = function (idCampoForm) {
    ...
    data = (idCampoForm === 'importoCDversamenti') ? $('#dataCDversamenti').val() : $('#dataDisposizione').val();
    var url = '/trasferimentoManuale.do', // + '?cf='+ Math.random();
        parameters = {
            importo: importo,
    ...
    AJAXCallPost(url, disponiTrasferimentoCDSuccess, disponiTrasferimentoCDFailure, parameters);
};
...
var disponiTrasferimentoCC = function (idCampoForm) {
    ...
    AJAXCallPost(url, disponiTrasferimentoCCSuccess, disponiTrasferimentoCCFailure, parameters);
};
...
var disponiTrasferimentoCDER = function () {
    ...
    AJAXCallPost(url, disponiTrasferimentoCDERSuccess, disponiTrasferimentoCDERFailure, parameters);
};
...
var disponiTrasferimentoCC2 = function () {
    ...
    AJAXCallPost(url, disponiTrasferimentoCC2Success, disponiTrasferimentoCC2Failure, parameters);
    disableButton($('#disponiCC2a'));
};
...
var animazioneCCDerDiventa = function (tipo) {
    ...
    if (tipo === 'CDER') {
        ...
        if (margineCC < importoCDERVal) {
            importoCDER.val(margineCC.toString().replace('.', ','));
        }
    } else if (tipo === 'CC2') {
        importoCC2Val = importoCC2.val();
        importoCC2Val = (importoCC2Val === '') ? 0 : parseFloat(formattaNumeriInput(importoCC2Val));
        if (margineCD < importoCC2Val) {
            importoCC2.val(margineCD.toString().replace('.', ','));
        }
    }
};
...
var animazioneCCCDdiventaTr = function (tipo) {
    ...
    if (tipo === 'CD') {
        ...
        if (margineCC < importoCDVal) {
            ...
            importoCDVal = dashboardData.contoCorrente.margine;
        }
        ...
        if (margineCD < importoCCVal) {
            ...
            importoCCVal = dashboardData.contoDeposito.margine;
        }
...
var checkTrasfList = function () {
    ...
    $('#contentTrasf6').unbind('content:caricaListaTRicComplete');
...
var disponiEliminaTrasferimento = function () {
    var disponi3 = $('#disponi3'),
        ...
        url = '/eliminaTRic.do', // + '?cf='+ Math.random();
        parameters = {
            cd: dashboardData.cd,
            id: trasferimenti.lista[index][0]
        };
    AJAXCallPost(url, disponiEliminaTrasferimentoSuccess, disponiEliminaTrasferimentoFailure, parameters);
    disableButton(disponi3);
};
...
var openTrasferimentoRicorrente = function () {
    ...
    var importoTRf = $('#importoTRf'),
        ...
        scadenza = (dashboardData.tipoRapporto === 2) ? readRadio('ricorrentiForm', 'scadenzaTRf') : readRadio('scadenzeLabeleRadio', 'scadenza'),
        numero = numeroTRf.val(),
        checkImporto = isNumber(importo) && !isZero(importo),
        checkData = (isValidDate(aPartireDa)),
        checkHolidays = (checkData && isFutureDate(aPartireDa) && noHolidays(getDateFromString(aPartireDa))[0]),
        checkNumero = true,
        errors = [];
    if (scadenza === 'dopo') {
...
var disponiTrasferimentoRicorrente = function () {
    ...
    var url = '/trasferimentoRicorrente.do', // + '?cf='+ Math.random();
        parameters = {
            cc: dashboardData.cc,
            cd: dashboardData.cd,
    ...
    AJAXCallPost(url, disponiTrasferimentoRicorrenteSuccess, disponiTrasferimentoRicorrenteFailure, parameters);
...
var popolaTabellaTS = function () {
    ...
    $.each(trasferimenti.lista, function (index, objConto) {
        ...
        var htmlTD = '<tr class="' + classTR2 + ' bb classeFalsa">' +
            ...
            '<td class="center ' + classTR + '"><a href="#!" class="btn-icon eliminaTrasferimento" data-index="' + index + '"><i class="icon icon-delete_table"></i></a></td>' +
            '</tr>';
    ...
    if (isValidDate(data) && isNumber(numeroVers) && isFutureDate(data)) {
        ...
        if (dopoN.checked) {
            var dataScadenza = '',
                ...
                mm = data.substr(3, 2),
                gg = data.substr(0, 2),
                dataDate = new Date(aaaa, parseFloat(mm) - 1, gg);
...
var loadEliminaLV = function () {
    var index = $('#disponi6').attr('data-index'),
        ...
        boxRetentionLV = $('#boxRetentionLV');

    boxRetentionLV.html();
    if (viewBannerRetention(vincolo[7], vincolo[1])) {
        boxRetentionLV.load(
            createAJAXUrl('/getBannerRetention.jsp', '/wbOnetoone/wbblack/ajax/json'),
            { path: "RIA_SVINCOLA" },
            function () {
                boxRetentionLV.show();
            }
...
var disponiEliminaLV = function () {
    ...
    AJAXCallPost(url, disponiEliminaLVSuccess, disponiEliminaLVFailure, parameters);
...
var checkSvincoloInput = function () {
    ...
    var totVincolato = $.trim($('#valoreInizialeLV').html()),
        el = $('#parteLV');
...
var openNuovaLV = function (id, isNonSvinc, minVinc, nomeNonSvinc) {
    ...
    if (isNumber(valore)) {
        ...
        // Non permetto che si possa vincolare un importo maggiore della disponibilit� in CD
...
var fillFormNuovaLV = function (vincolo) {
    ...
    var durata = vincolo.durataVinc,
        ...
        guadagno = formattaNumeriInput(calcolaGuadagnoNetto(valore, vincolo.moltiplicatore)[0]);
    disponi7.attr('data-codice', vincolo.codice);
    disponi7.attr('data-durata', vincolo.durata);
    $('#numeroLV').html(vincolo.dataa);
    $('#dataCreazioneLV').html(vincolo.dataa);
    $('#tassoLV').html(tasso);
    $('#valoreVincolatoLV').html(formattaNumOutput(valore));
    $('#dataScadenzaLV').html(vincolo.datas);
    $('#durataLV').html(durata);
    $('#valoreFinaleLV').html(formattaNumeriOutput(parseFloat(formattaNumeriInput(valore)) + parseFloat(guadagno)));
...
var disponiNuovaLV = function () {
    var url = '/creaLVinc.do', //+ '?cf='+ Math.random();
        disponi7 = $('#disponi7'),
        inputcont = $('#confermaContent7').find('.inputcont:first'),
    ...
    AJAXCallPost(url, disponiNuovaLVSuccess, disponiNuovaLVFailure, parameters);
...
var disponiNuovaLVSuccess = function (data) {
    ...
    if (isCallFailed(data)) {
        ...
        if (data.isDescrSaved) message = data.errore;
        else {
            message = 'Per motivi tecnici, non � stato possibile memorizzare il nome della linea.<br/>Riprova pi� tardi.';
        }
    }
...
var popolaTabLV = function () {
    ...
    contentExtraMoney.hide();
    nonSvincText.hide();
    svincText.show();
    helpEM.unbind('click');

    var vincoliInd = -1,
        wrap = '<div class="col-sm-4 col-md-3"></div>';
    $.each(vincoli, function (key, val) {
        vincoliInd += 1;
        var div = 'offertaLV',
            barrato = '',
        ...
        if (val.isFresh === 'S') {
            ...
            tasto = '<div class="vincolaLV">' + promo + '<span data-id="' + vincoliInd + '" data-nonsvinc="' + isNonSvinc + '" data-nomenonsvinc="' + nomeNonSvinc + '" data-minvinc="' + minVinc + '" class="btnLV">APRI ORA</span></div>';

            tassoTooltip = tasso;

            if (isBase && !isMissingData(val.testTooltip)) {
                tassoTooltip = '<span class="tooltip"><a href="#!" onclick="return false;">' + tasso + '<span><span><span>' + val.testTooltip + '</span></span></span></a></span>';
            }
            ...
            html_tab = '<div id="boxLargeLV' + vincoliInd + '" class="' + div + '" title="' + vincTitle + '">' +
                ...
                promozione +
                offerta +
                '<div class="percentuale" id="parcentualeLV' + vincoliInd + '">' + tassoTooltip + '<span>&#37;</span></div>' +
                barrato +
            ...
            boxTabLineeVincolate.append($(wrap).append(html_tab));
        }
    });

    if (contentExtraMoney.css('display') !== 'none') {
