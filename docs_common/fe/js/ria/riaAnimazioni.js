...
var toggleTabUnloadCallback = function (event) {
    ...
    var content1 = $('#content' + toggleTabConf.suffix + toggleTabConf.index).unbind('content:unloadComplete'),
    ...
    if (toggleTabConf.hasEffects) {
        if (toggleTabConf.effectName !== 'appear') {
            content1.slideUp(1000, function () {
                content1.removeClass(contentClass);
            });
        } else {
...
var toggleTabLoadCallback = function (event) {
    ...
    var tab2 = $('#tab' + toggleTabConf.suffix + toggleTabConf.num),
        content2 = $('#content' + toggleTabConf.suffix + toggleTabConf.num).unbind('content:loadComplete'),
        contentClass = getContentClass(toggleTabConf.suffix);
    if (tab2.length) {
        if (!tab2.hasClass('on')) {
            tab2.addClass('on');
...
var changeItemAccordion = function () {
    if (!isMoving) {// Effect.Queues.get('effects').length === 0
        ...
        if (doUnload) {
            content = $('#contentAccordion' + changeItemAccordionConf.index);
        ...
        if (doLoad) {
            content = $('#contentAccordion' + changeItemAccordionConf.num);
...
var contentProgressAccordionCallBack = function (event) {
    ...
    var content = $('#contentAccordion' + contentInd);
