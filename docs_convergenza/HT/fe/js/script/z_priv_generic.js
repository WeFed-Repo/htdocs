...
function setOverlayerIconaOpertiva(contIconaOper) { /* Tolto parametro modalTitle perch̩ non usato */
...
var appendDatePickerIcon = function(idElement) {
    ...
    //se ̬ android
        ...
        inputDate.on('keydown', function(event) {
            var valueInputDate = inputDate.val();
            if (valueInputDate.length == 2 && event.keyCode != 8) {
                inputDate.val(valueInputDate + '/');
            } else if (valueInputDate.length == 5 && event.keyCode != 8) {
                inputDate.val(valueInputDate + '/');
            }
        });
    }
        ...
        highlightTick = function(value) {
            if (settings.hasHighlight) {
                ...
                if (value === settings.min) {
                    ...
                    prevenSlider.find('.sliderTicks .sliderTickMilestone').eq(Math.round((value - settings.min) / settings.milestone)).addClass('sliderTickOn');
                }
            }
        };
        setText = function(value) {
            if (settings.hasText) {
                sliderText.html(value);
            }
        };
        ...
        return this.each(function() {
            ...
            setDimensions = function() {
                ...
                if (vertical) {
                    ...
                    $carouselPlus.addClass('vertical').outerHeight(height);
                } else {
...
var initRadioInput = function () {
    $('.radioInput').each(function () {
        ...
        radioInput.find('input[type="radio"]').click(function () {
            ...
            // Controllo se c'̬ un input associato a questo radio button
            if (input.length) {
...
$(function() {
    ...
    if ($("body").hasClass("webview")) $("body").removeClass("loading").fadeIn(300);
