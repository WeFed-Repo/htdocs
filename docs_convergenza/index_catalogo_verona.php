...
<head>
	...
	@media (min-width:1024px){
		#mainContent {padding:20px!important;overflow:hidden;}
	...
	#main {margin:0 auto; max-width:1110px;}
	#main .wrapper {
    overflow: visible!important;
    }
	header {background:#fff;width:100%;height:70px;}
	h1.librerie {border-bottom:0; box-shadow: none; font-size:25px; color: #fff;padding: 15px 15px;font-weight: bold;margin: 0;font-size: 32px;float:left;}
	h1.librerie a {text-decoration:none;}
	h1.librerie span {color:#000;}
	#mainContent {background:#fff;padding:20px}
	h2.oggettolib {color:#fff; background-color:#666; padding:5px; font-weight:bold;margin-bottom:20px;margin-top:10px;}
	a.libBtn {width:32px;height:32px; margin: 0 10px 0 5px;color:#fff; text-align:center; line-height:32px;padding:0;border-radius:50%;display:inline-block; border: solid 2px #666;border-radius:50%;color:#000;}
	a.libBtn img {display:inline-block; margin-top:5px;}
	.container{width:100%!important;padding:0;}

	/* Switch */
	#lib-switch {float:right;padding:15px;}
	#lib-switch a.pulsante {width:40px;height:40px;background:#999;border-radius:50%;line-height:40px;margin:0 0 0 15px;text-align:center;display:block;color:#fff;float:left;}
	#lib-switch a.pulsante:hover {text-decoration:none}
	#lib-switch a.pulsante.webank {background:#8ab10b;}
	#lib-switch a.pulsante.youweb {background:#007c66;}
	...
	header{background-color: #fff!important;position:relative!important;}
	...
	<?php
   	 ...
   	 if ($site == 'youweb') {
			print '<link media="screen" rel="stylesheet" type="text/css" href="/HT/fe/css/youWeb-struttura.css?v=161229"  />
			<link media="screen" rel="stylesheet" type="text/css" href="/HT/fe/css/youweb_generic.css?v=161229"  />';
		}
	 else {
	...
	<script type="text/javascript" src="common/fe/js/funzioni_convergenza.js?r="></script>
...
<!-- FINE FUNZIONI IN PAGINA -->
	<div id="main">
	<header>
		<h1 class="librerie"><a href="<?php print $libsurl?>/index.php?site=<?php print $libs ?>" class="libBtn">&lt;</a>Librerie Convergenza - <?php print($tipo); ?></h1>
		<div id="lib-switch">
			<a href="<?php print $libsurl?>./index_catalogo_verona.php?tipo=<?php print($tipo); ?>&site=webank" class="pulsante webank">Wb</a>
			<a href="<?php print $libsurl?>./index_catalogo_verona.php?tipo=<?php print($tipo); ?>&site=youweb" class="pulsante youweb">Yw</a>
		</div>
	</header>
	<div class="wrapper">
		<!-- MAIN WRAPPER -->
			...
			<?php
							...
							$basefolder = "include/oggetti/".$tipo."/Verona";
							$elencocart = array();  
									...
									<div class="col-xs-12">
										<h2 href="/index_catalogo.php?tipo=<?php print $tipooggetto; ?>" class="oggettolib"><?php print $tipooggetto; ?>
											<span style="float:right;font-size:14px">
												<?php 
												if ($tipo != "Widget" && $tipo != "Widget home") {
												print "include/oggetti/".$tipo."/Verona/".$tipooggetto."/oggetto.php" ?> 
												<a href="javascript:;" onclick="javascript: windowdetail('<?php print "include/oggetti/".$tipo."/Verona/".$tipooggetto."/oggetto.php" ?>');" style="color:#fff">(Sorgente)</a>	
												
												<?php }?>
												</span>
										</h2>
										<?php virtual ("include/oggetti/".$tipo."/Verona/".$tipooggetto."/oggetto.php");
										?>
									</div>
								</div>
								<!-- Fine blocco oggetto -->
