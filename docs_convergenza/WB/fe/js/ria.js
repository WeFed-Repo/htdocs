/* jshint strict:false */
/* global $, AmCharts, isMissingData, cgi_script, console, pathLibrerie, links,
	...
	getContentClass, caricaDashboard, caricaSessioneCarte, changeItemAccordionConf, selectTab,
...
/* exported AJAXCallPost, AJAXCallGet, initLinkStatici, initEventi, loadContentRia1, loadContentRia1Callback,
	...
	openTutorial, resetTAB, extraMoneyOnOff, cambiaConto, cambiaContoMobile */
...
var graficoConti1, graficoConti2,
    graficoConti1Mobile, graficoConti2Mobile;
// ********** Utility AJAX **********
var AJAXCallPost = function (AjaxUrl, successFunc, failureFunc, parameters) {
    parameters.BV_UseBVCookie = 'Yes';
    AJAXCall('post', AjaxUrl + '?cf=' + Math.random(), successFunc, failureFunc, parameters);
};
var AJAXCallGet = function (AjaxUrl, successFunc, failureFunc, parameters) {
    parameters.cf = Math.random();
    parameters.BV_UseBVCookie = 'Yes';
    AJAXCall('get', AjaxUrl, successFunc, failureFunc, parameters);
};
var AJAXCall = function (method, AjaxUrl, successFunc, failureFunc, parameters) {
    $.ajax({
        type: method,
        dataType: 'json',
        url: AjaxUrl,
...
$(function () {
    ...
    // Sistema la stampa della modale del mio patrimonio
    appendModalToBody($('#mioPatrimonioModal'));
...
var initLinkDinamici = function (contoSelezionato) {
    ...
    if (!isMissingData(dashboardData.cd)) {
        $('#boxExcelLV').attr('href', links.excelLV + '&contoSelVincolo=' + dashboardData.cd);
    }
    if (!isMissingData(dashboardData.ct)) {
        dettCTLink = links.dettLiquiditaCT + "&CC=" + dashboardData.ct;
        ...
        if (!isMissingData(dashboardData.cder)) {
            ...
            $('.dispTrasfCDERLink').attr('href', dettCDERLink);
            $('#dettaglioCDERLink').attr('href', dettCDERLink);
        }
    } else {
        if (!isMissingData(dashboardData.cder)) {
            dettCCLink = links.dettLiquiditaCC + "&codCC=" + formatter(dashboardData.cc);
            dettCDERLink = links.dettLiquiditaCCDER + "&codCC=" + formatter(dashboardData.cder);
    ...
    if (!isMissingData(dashboardData.cc)) {
        ...
        if (!isMissingData(dashboardData.cd)) {
            var dettCDLink = links.dettLiquiditaCD + "&codCC=" + formatter(dashboardData.cd);
...
var initEventi = function () {
    ...
    $('#disponi13').click(function () {
        var disponi13 = $(this);
        if (!disponi13.hasClass('btn-disabled')) {
    ...
    if (conti.length === 1) {
        $('#riaDesktopWrap').prepend($('#riaMobileWrap').find('strong').clone()).removeClass('loading');
    }
    ...
    $(window).resize(function () {
        ...
        // Mobile
        if (graficoConti1Mobile && graficoConti1Mobile.invalidateSize) {
            graficoConti1Mobile.invalidateSize();
        }
        if (graficoConti2Mobile && graficoConti2Mobile.invalidateSize) {
            graficoConti2Mobile.invalidateSize();
        }
    }).resize();
};

var moveSelectConti = function () {
    var $riaMobileWrap = $('#riaMobileWrap'),
        $riaDesktopWrap = $('#riaDesktopWrap'),
        $selectConti = $('#selectConti'),
        $inputGroupAddon,
        isSmartPhone = window.innerWidth < 768;

    if (isSmartPhone && !$riaMobileWrap.find('#selectConti').length) {
        $riaDesktopWrap.addClass('loading');
        $inputGroupAddon = $riaDesktopWrap.find('.input-group-addon').detach();
        $riaMobileWrap.prepend($inputGroupAddon).prepend($selectConti.detach()).removeClass('loading');
    } else if (!isSmartPhone && !$riaDesktopWrap.find('#selectConti').length) {
        $riaMobileWrap.addClass('loading');
        $inputGroupAddon = $riaMobileWrap.find('.input-group-addon').detach();
        $riaDesktopWrap.prepend($inputGroupAddon).prepend($selectConti.detach()).removeClass('loading');
    }
};
...
var initRIA = function () {
    ...
    contenuti545.add('.modal').add('#riamobile').find('.valuta').html(valutaHTML);
...
var getAccordionContent = function (index) {
    var $content;

    switch (index) {
        case 1:
            $content = $('.boxSaldoCC .collapse-bottom');
            break;
        case 2:
            $content = $('.boxSaldoCD .collapse-bottom');
            break;
        case 3:
            $content = $('.boxSaldoLV .collapse-bottom');
            break;
        case 4:
            $content = $('.boxSaldoCDER .collapse-bottom');
            break;
    }
    return $content;
};
var getContent = function (suffix, index) {
    return suffix === 'Accordion' ? getAccordionContent(index) : $('#content' + suffix + index);
};
var fireEventOnContent = function (prefix, suffix, index) {
    getContent(suffix, index).trigger("content:" + prefix, [suffix, index]);
};
var stopObservingEventOnContent = function (prefix, suffix, index) {
    getContent(suffix, index).off('content:' + prefix + 'Complete');
};
...
var loadContentRia1 = function () {
    ...
    // NON SERVE? caricaSessioneCarte();
};
var loadContentRia1Callback = function () {
    var prefix = 'load',
        indexLiquidita = 1,
        accordionSaldi = $('#accordionSaldi');
    resetErrorDiv('errorDiv1', ['etichettaCD', 'etichettaCC']);
    ...
    if (dashboardData.tipoRapporto === 4 || dashboardData.tipoRapporto === 6) {
        $('#accordionSaldi .boxSaldoCC').closest('.col-sm-12').removeClass('col-md-6').addClass('col-md-12');
    } else {
        $('#accordionSaldi .boxSaldoCC').closest('.col-sm-12').removeClass('col-md-12').addClass('col-md-6');
    }
...
var afterLoadContentLiquidita1 = function () {
    ...
    if (dashboardData.tipoRapporto <= 2) {
        ...
        // PIE CHART 1 MOBILE
        graficoConti1Mobile = AmCharts.makeChart("graficoContiMobileDiv", json[0]);
        // PIE CHART 2 MOBILE
        graficoConti2Mobile = AmCharts.makeChart("graficoContiMobileDivInner", json[1]);
    }
...
var extraMoneyOnOff = function (saldoFresh) {
    ...
    if (saldoFresh > 0) {
        $('#apriOraEM').attr('src', '/WB/fe/img/ria_btn_apriora.gif');
        ...
        $('#apriOraEM').attr('src', '/WB/fe/img/ria_btn_apriora_emoff.gif');
        $('#saldoEM').html(formattaNumero(0));
        importoNuovaEM.attr('disabled', true);
        nuovaEM.addClass('noEM').off('click');
...
var resetCCCDdiventaTr = function () {
    var margineCC = Math.max(parseFloat(dashboardData.contoCorrente.margineTrasf), 0);
    var margineCD = Math.max(parseFloat(dashboardData.contoDeposito.margine), 0);
    $('.diventaCCTr').html(formattaNumero(margineCC));
    $('.diventaCDTr').html(formattaNumero(margineCD));
