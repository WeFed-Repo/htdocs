...
<style type="text/css">
	...
	function visualizzaSegnalazione() {
	 ...
	 function inserisciNuovoModelloStepForward() {
      ...
      if (!getSubmitted()) {
	      document.f24MainForm.action = '/sapef24/f24/inserisciNuovoModelloStepPhone.do?id_chiamata=1583168303704_01IHF24_Zxq7W6RUtxkgwfbd&_control.tipo=S&indiceConto='+indiceConto+'&indiceContribuente='+indiceContribuente;
...
<form name="rubricaContribuentiForm" method="post" action="/sapef24/f24/aggiungiContribuente.do" class="form-horizontal"><input type="hidden" name="org.apache.struts.taglib.html.TOKEN" value="11e18f42020b666f5098474d7dabd470">
...
<input type="hidden" name="id_chiamata" value="1583168303704_01IHF24_Zxq7W6RUtxkgwfbd">
...
<script type="text/javascript">
 ...
 var transictionRubrica = false;
 var falloUno = false;
 $('#myCarouselRubrica').on('slid.bs.carousel', function () {
		indiceContribuente = $(this).find(".active").index();
		transictionRubrica = false;
	 });
  ...
  $(function() {
	    ...
	    $( "#sliderRubricaId" ).css( {
  		  "margin-left" : "45px",
  		  "margin-right" : "45px" });
	    var sliderRubrica = $( "#sliderRubricaId" ).slider({
	      min: 0,
	      max: 1 - 1,
	      range: "min",
	      value: 0,
	      ...
	      stop: function( event, ui ) {
	    	$("#myCarouselRubrica").carousel(ui.value);
	    	updateSliderRubrica(ui.value);
	      }
	    });
	...
	if( false  && ! falloUno ){
		...
		if ((transictionRubrica == false) && (indiceContribuente > 0)) {
	        ...
	        $("#scrollPrevRubricaId").attr("href", "#myCarouselRubrica");
		} else {
			//No more previous
  ...
  function nuovoModello() {
  	 	mostra_loading_mobile();
      if (!getSubmitted()) {
   	      document.forms[0].action = '/sapef24/f24/inserisciNuovoModelloPhone.do?_control.tipo=S';
	...
	function inserisciNuovoModello(indice) {
		...
		document.forms[0].action = './redirectModuloF24.html?id_chiamata=1583168303704_01IHF24_Zxq7W6RUtxkgwfbd&_control.tipo=' + tipoModelloCompilazione[0] + "&modulo=" + tipoModelloCompilazione[1] + "&indexRubrica=" + indice + "&function=0";
		document.forms[0].submit();
	}
