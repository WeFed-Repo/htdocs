...
//	wrp_script_direct="/webankpri/goToInvestDirect";
	...
	var URL_SCHEDA_FONDO=wrp_script + "/wbOnetoone/2l/action/investywb/RicercaFondi.action?OBSCNT=FNZ_NAV&tabId=nav_priv_wbx_investimenti&OBSKEY=nav_priv_wbx_acquisto_new&fondo=true&codiceInterno=";
	...
	var getTableOrdini = function() {
		...
		$.ajax({
			url : URL_RICERCA + Math.random() ,//"/librerie/include/commons/ajax/sitOrdini.json?a="	+ Math.random(),
			dataType : "json",
						...
						if (200 == jqXHR.status) {
							if(resp.data.action == "success") {
								...
								$.each(resp.data.socGest, function(i, value) {
									...
									lsSocGest += "<img src=\"" +cmn_web_context+ value.ImgPath + "\" class=\"img100\">";// /common
