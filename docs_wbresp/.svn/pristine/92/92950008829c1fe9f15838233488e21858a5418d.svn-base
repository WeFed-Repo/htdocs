function sendotp() {
	var parameters = 'comando=sendOTP&' + 'userName=' + $('#userName').val();

	AJAXGenericCall('POST', '/aol/login.do',
	...
	function(data) {
		...
		if (data.isEsito) {
			...
			// Attivo overlay per esito
			showError(data.descr);
		}
	},
...
function validaotp() {
	var parameters = 'comando=validaOTP&' + 'userName=' + $('#userName').val() + '&codOtp=' + $('#otp').val();
	AJAXGenericCall('POST', '/aol/login.do',
	...
	function(data) {
		...
		if (data.isEsito) {
			...
			showError('Codice di controllo errato');
		}
	},
