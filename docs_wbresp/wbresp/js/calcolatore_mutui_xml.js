		...
		success: function(data) {
			...
			$.each(mutui,function(i,v){
				...
				if (mutuo.attr("disponibile") == "y")
				{
					...
					if (!isDetail) mNodeHead.append($("<a>").addClass("scheda hidden-xs").html("Scheda prodotto e Informazioni Generali").attr({"href": mutuo.find("link").text(), "target": "_blank"}));
					mrata = Number(mutuo.attr("rata"));
					if (mrata > ratamax) ratamax = mrata;
					
					mGauge = $("<div>").addClass("mGauge").append($("<span>").addClass("rata").html("<span class='smaller hidden-xs'>La tua rata: </span>" + formatMigliaia(mutuo.attr("rata").split(".")[0]) + ",<span class=\"small\">"+ mutuo.attr("rata").split(".")[1]+"&nbsp;&euro;</span><span class=\"smaller\">&nbsp;al mese</span>"));
					mGauge.attr("w",mrata);
					...
					mNodeLeft.append($("<a>").addClass("scheda" + ((!isDetail) ? " hidden-sm hidden-md hidden-lg":"")).html("Scheda prodotto e Informazioni Generali").attr({"href": mutuo.find("link").text(), "target": "_blank"}));
					...
					if (isDetail)
					{
						...
						mNodeLeft.append($("<a>").addClass("scheda").html("Piano di ammortamento").attr({"href": mutuo.find("pianoammortamento").text(), "target": "_blank"}));
						// Assicurazioni
