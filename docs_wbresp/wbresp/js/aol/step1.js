/* globals $:false, setFormGroupCell:false, setFormGroupDate:false, arePartiallyCompiled:false,
    ...
    $(function() {
        ...
        $('#btnSubmit').click(function () {
            ...
            //CONTROLLO AMERICANITA e solo dopo check del Form
            checkAmericanita();
            //checkForm(formEl);
        });
        ...
        var checkAmericanita = function() {
            var isTinReq = false;
                //reset di eventuali box di alert
                 $(".boxalert.americano").remove();
               
            //ARRAY DA CONTROLLARE: NASCITA USA; CITTADINANZA USA E SEZIONE RESIDENZA AGGIUNTIVA USA
            var isNatoAmericano = [false,false,false],
                isCittadinanzaAmericana = [false,false,false],
                isFatcaFieldFilled = [false,false,false];

            //funzione per popolare gli array da controllare
            var handlePushArray = function(indexArray,elementArray,boolean) {
                elementArray[indexArray] = boolean;
            }
            //CICLO PER TUTTI GLI INTESTATARI
            for (i = 1; i <= numInt; ++i) {
                var indexArray = i-1;
        ...
        //al change dello stato di nascita popolo automaticamente la cittadinanza se necessario
        var checkValueNascita = function() {
            for (i = 1; i <= numInt; ++i) {
                if( $("#stato_nascita_int" +i).val().indexOf("STATI UNITI")!=-1) {
                    impostaValoreSelect('cittadinanza_int'+i, 'STATI UNITI D\'AMERICA');
                    $("#cittadinanza_int" +i).attr("disabled",true).addClass("cittadinanza-americana-disabled");
                   
                }
                else if ($("#stato_nascita_int" +i).val().indexOf("STATI UNITI")===-1 && $("#cittadinanza_int" +i).is(":disabled")) {
                    impostaValoreSelect('cittadinanza_int'+i, 'ITALIA');
                    $("#cittadinanza_int" +i).attr("disabled",false).removeClass("cittadinanza-americana-disabled");;
                }
            }
        ...
        for (i = 1; i <= numInt; ++i) {
            $("#stato_nascita_int" +i).on("change",checkValueNascita)
        }
