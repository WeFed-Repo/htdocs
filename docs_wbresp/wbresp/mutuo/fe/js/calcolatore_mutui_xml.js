...
function mCheckVal(field,fv)
{
	...
	/* Blocco valore immobile solo per Banco */
	if (field.attr("id") == "mValore" && fv > 2000000) {
		field.val(formatMigliaia(2000000));
		errField = field;
		errMsg = "La polizza casa utilizzata per la simulazione online prevede un valore di ricostruzione massimo pari a 2.000.000&euro;. Verifica in filiale le opzioni di assicurazione del tuo immobile.";
		noerror = false;
	}
	...
	$.ajax({
		...
		success: function(data) {
			...
			$.each(mutui,function(i,v){
					...
					if (mutuo.attr("messaggioGenerico") =='errore generico') {
						erroreGenericoMsg = mutuo.find("messaggio").text();
					}
					...
					if (isDetail) {
						...
						textDesc = mutuo.find(">descrizione").text()
						textTitle = mutuo.find("nome").text()
						textPayoff = mutuo.find("sottotitolo").text()
						imgName = "/wbresp/mutuo/fe/img/mutuo_" +  mForm.find("input[name=finalita]").val().toLowerCase() + "_" + mutuo.attr("codicemutuo").split(" ").join("").toLowerCase();
					}
					else
					{
					...
					{	
						...
						var tassiTestoEsteso = $("<span>").addClass("testo-esteso").append($("<p>").html(
							"Istruttoria:  <span class=\"nowrap\">"+ mutuo.find("istruttoria").text() + " &euro;</span> " +
							"- Perizia:  <span class=\"nowrap\">"+ mutuo.find("perizia").text() + " &euro;</span> " +
							"- Incasso rata: <span class=\"nowrap\">"+ mutuo.find("incassorata").text() + " &euro;</span> " +
							"- Polizza casa*:  <span class=\"nowrap\">"+ mutuo.find("polizza").text() + " &euro;</span> " + 
							"- Imposta sostitutiva: <span class=\"nowrap\">"+ mutuo.find("imposta").text() + " &euro;</span> " + 						
							"- Costo totale del credito: <span class=\"nowrap\">"+ mutuo.find("costototcred").text() + " &euro;</span> " + 
							"- Importo totale dovuto dal Consumatore: <span class=\"nowrap\">"+ mutuo.find("imptotcons").text() +" &euro;</span>"),
						...
						if (isDetail) mNode.append($("<a>").addClass("scheda").html("Piano di ammortamento").attr({"href": mutuo.find("pianoammortamento").text(), "target": "_blank"}));
						...
						if (isDetail)
						{
							...
							*/
							...
							$("input[name='mutuo_nomeProdotto']").val(mutuo.find("nome").text());
							$("input[name='mutuo_importoRata']").val(mutuo.attr("rata"));
							$("input[name='mutuo_indice']").val(mutuo.find("indice").text());
							$("input[name='mutuo_spread']").val(mutuo.find("spread").text());
							$("input[name='mutuo_tasso']").val(mutuo.find("tasso").text());
							$("input[name='mutuo_taeg']").val(mutuo.find("taeg").text());
							$("input[name='mutuo_istruttoria']").val(mutuo.find("istruttoria").text());
							$("input[name='mutuo_perizia']").val(mutuo.find("perizia").text());
							$("input[name='mutuo_incassoRata']").val(mutuo.find("incassorata").text());
							$("input[name='mutuo_polizza']").val(mutuo.find("polizza").text());
							$("input[name='mutuo_imposta']").val(mutuo.find("imposta").text());
							$("input[name='mutuo_costoTotaleCredito']").val(mutuo.find("costototcred").text());
							$("input[name='mutuo_importoTotaleConsumatore']").val(mutuo.find("imptotcons").text());

							}

							if (!isDetail)
							{
								mScegli = $("<a>").addClass("btn-whlit btn-whlit-arrow btn-grlit btn-blacktxt btn-fs14 btn-gray-border uppercase").html("Scegli").attr({"href":"javascript:;", "codMutuo" : mutuo.attr("codicemutuo")});
						...
						else
						{
							...
							if (!mPromoMode) {
								var msg = (mutuo.find("messaggio").text().length>0) ? mutuo.find("messaggio").text() :  "Prodotto non disponibile per i parametri selezionati";
