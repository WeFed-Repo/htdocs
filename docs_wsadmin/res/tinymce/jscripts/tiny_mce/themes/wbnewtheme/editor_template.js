 ...
 */
	...
	function checkInnerHTML(innerHTML, selectedText) {
		
		var startTag = "<span>";
		var endTag = "</span>";

		var c = innerHTML.split(startTag);
		var i, l, pos,sub,end;
		var textArray = new Array();

		for (i=0, l=c.length; i<l; i++) {
			if (c[i] != '') {
				pos = c[i].indexOf(endTag);
				if (pos != -1) {
					sub = c[i].substring(0,pos);
					end = selectedText.indexOf(sub);
					if (end == 0) {
						end = sub.length;
						textArray[0] = selectedText.substring(0,end);
						textArray[1] = selectedText.substring(end);
						i = l;
					}
				}
			}
...
/* bp74467 */
/* Funzioni label tassi */
/* Macro combobox elements */
var macrosLabels = new Array('-- Macro --', 'Tasso BCE', 'Tasso Conto@me under 50.000', 'Tasso Conto@me over 50.000', 'Tasso MyBusiness under 50.000', 'Tasso MyBusiness over 50.000', 'Tasso Conto@me Trader under 50.000', 'Tasso Conto@me Trader over 50.000', 'Tasso Conto@me Trader Fido', 'Tasso Conto@me Extra Fido', 'Tasso MyBusiness Extra Fido', 'Baloon','Data Ultima Modifica Tasso','Tasso ISC','Tasso TAN', 'Costo Utilizzo', 'Delta Tasso Fido', 'Data Modifica Fido', 'Tasso Fido', 'Costo Utilizzo Fido', 'Tasso ISC Me', 'Delta Tasso ISC', 'Importo Utilizzo' );
var macrosIDs = new Array('', 'tasso', 'tassoMeInf', 'tassoMeSup', 'tassoMyBusinessInf', 'tassoMyBusinessSup', 'tassoMeTraderInf', 'tassoMeTraderSup', 'tassoMeTraderFido', 'tassoMeExtraFido', 'tassoMyBusinessExtraFido', 'baloon', 'dataUltimaModifica', 'tassoISC', 'tassoTAN', 'utilizzo', 'deltaFidoMe', 'dataModificaFido', 'tassoFido', 'costoUtilizzoFidoMe', 'tassoIscMe', 'deltaIsc', 'importoUtilizzo' );
var macrosScripts = new Array(
  '',
  '[tassoBCE]',
  '[tassoMeInf]',
  '[tassoMeSup]',
...
var macrosCnt = macrosIDs.length;
/* bp74467 */

/* Import theme specific language pack */
tinyMCE.importThemeLanguagePack('wbnewtheme');

var TinyMCE_WbnewthemeTheme = {
	...
	_buttons : [
		...
		['tooltip', 'tooltip.gif', 'lang_tooltip_desc', 'Tooltip'],
		...
		['litag', 'litag.gif', 'lang_litag_desc', 'mcelitag', true],
	...
	_buttonMap : 'anchor,backcolor,bold,bullist,charmap,cleanup,code,copy,cut,forecolor,help,hr,image,indent,italic,justifycenter,justifyfull,justifyleft,justifyright,link,litag,newdocument,numlist,outdent,paste,redo,removeformat,strikethrough,sub,sup,tooltip,underline,undo,unlink,visualaid,advhr,ltr,rtl,emotions,flash,fullpage,fullscreen,iespell,insertdate,inserttime,pastetext,pasteword,selectall,preview,print,save,replace,search,table,cell_props,delete_col,delete_row,col_after,col_before,row_after,row_before,merge_cells,row_props,split_cells',
	...
	getControlHTML : function(button_name) {
		...
		for (i=0; i<TinyMCE_WbnewthemeTheme._buttons.length; i++) {
			var but = TinyMCE_WbnewthemeTheme._buttons[i];
		...
		switch (button_name)
    {
      /* bp74467 */
      case "macroselect":
				var html = '<select id="{$editor_id}_macroSelect" name="{$editor_id}_macroSelect" onfocus="tinyMCE.addSelectAccessibility(event, this, window);" onchange="tinyMCE.execInstanceCommand(\'{$editor_id}\',\'MacroBlock\',false,this.options[this.selectedIndex].value);" class="mceSelectList">';

				// Build format select
				for (var i = 0; i < macrosCnt; i++)
        {
          html += '<option value="&lt;' + macrosIDs[i] + '&gt;">' + macrosLabels[i] + '</option>';
				}
			...
			case "formatselect":
				...
				var formats = tinyMCE.getParam("theme_wbnewtheme_blockformats", "p,address,pre,h1,h2,h3,h4,h5,h6", true).split(',');
				var lookup = [
			...
			case "fontselect":
				...
				var fonts = tinyMCE.getParam("theme_wbnewtheme_fonts", nFonts).split(';');
	...
	execCommand : function(editor_id, element, command, user_interface, value)
  {
		switch (command)
    {
      ...
      case "MacroBlock":
          ...
          for (var i = 0; i < macrosCnt; i++)
          {
            if (macroID == macrosIDs[i])
              html += macrosScripts[i];
          }

          tinyMCE.execInstanceCommand(editor_id, 'mceSetContent', false, html);
        }
			...
			case "mceLink":
				...
				var href = "", target = "", title = "", onclick = "", action = "insert", style_class = "", style_rel="";
				...
				if (tinyMCE.linkElement) {
					...
					style_rel = tinyMCE.getAttrib(tinyMCE.linkElement, 'rel');
				...
				if (inst.settings['insertlink_callback']) {
					var returnVal = eval(inst.settings['insertlink_callback'] + "(href, target, title, onclick, action, style_class, style_rel);");
					if (returnVal && returnVal['href']) {
						TinyMCE_XbankthemeTheme._insertLink(returnVal['href'], returnVal['target'], returnVal['title'], returnVal['onclick'], returnVal['style_class'], returnVal['style_rel']);
					}
				} else {
					tinyMCE.openWindow(template, {href : href, target : target, title : title, onclick : onclick, action : action, className : style_class, relName : style_rel, inline : "yes"});
				}
			...
			case "mcelitag":
				...
				var href = "", target = "", title = "", onclick = "", action = "insert", lilabel = "";
				...
				if (tinyMCE.linkElement) {
					...
					title = tinyMCE.getAttrib(tinyMCE.linkElement, 'title');
					onclick = tinyMCE.getAttrib(tinyMCE.linkElement, 'onclick');
					lilabel = tinyMCE.linkElement.innerHTML;
					...
					var nodeSize = tinyMCE.selectedElement.childNodes.length;
					var flag = "false";
					var innerHtml = "";
					for (var index = 0; index < nodeSize; index++) {
						if (tinyMCE.selectedElement.childNodes[index].nodeName.toLowerCase() == 'a') {
							innerHtml = tinyMCE.selectedElement.childNodes[index].innerHTML.trim();
							index = nodeSize;
							if (innerHtml != '') {
								flag = "true";
							}
						}
						else if (tinyMCE.selectedElement.childNodes[index].nodeName.toLowerCase() == 'br'){
							tinyMCE.selectedElement.removeChild(tinyMCE.selectedElement.childNodes[index]);
						}
					}
					if (flag == "false") {
						innerHtml = tinyMCE.selectedElement.innerHTML;
					}
					if (innerHtml != null && innerHtml.indexOf("<li>") == -1) {
						innerHtml = innerHtml.replace("<br>", "").trim();
						lilabel = innerHtml;
					}
				}
				...
				if (inst.settings['insertlitag_callback']) {
					var returnVal = eval(inst.settings['insertlitag_callback'] + "(href, target, title, onclick, action, lilabel);");
					if (returnVal && returnVal['href']) {
						TinyMCE_XbankthemeTheme._insertLitag(returnVal['href'], returnVal['target'], returnVal['title'], returnVal['onclick'], returnVal['lilabel']);
					}
				} else {
					tinyMCE.openWindow(template, {href : href, target : target, title : title, onclick : onclick, action : action, lilabel: lilabel, inline : "yes"});
				}
		...
		case "Tooltip":
				...
				var text = "", action = "insert";

				if (tinyMCE.linkElement) {
					var innerHTML = tinyMCE.selectedElement.innerHTML;
					var textArray = checkInnerHTML(innerHTML, selectedText);
					text = textArray[0];
					selectedText = textArray[1];
					action = "update";
				}
				
				var template = new Array();

				template['file'] = 'tooltip.htm';
				template['width'] = 450;
				template['height'] = 200;
				...
				if (inst.settings['insertlink_callback']) {
					var returnVal = eval(inst.settings['insertlink_callback'] + "(text);");
					if (returnVal && returnVal['href'])
					
						TinyMCE_WbnewthemeTheme._insertTooltip(returnVal['text']);
				} else {
					tinyMCE.openWindow(template, {text : text, selectedText: selectedText, inline : "yes"});
				}
				return true;
			...
			case "mceAdvLink":
				...
				if (inst.settings['insertlink_callback']) {
					...
					if (returnVal && returnVal['href'])
						TinyMCE_WbnewthemeTheme._insertLink(returnVal['href'], returnVal['target'], returnVal['title'], returnVal['onclick'], returnVal['style_class']);
				} else {
			...
			case "mceImage":
				...
				if (inst.settings['insertimage_callback']) {
					...
					if (returnVal && returnVal['src'])
						TinyMCE_WbnewthemeTheme._insertImage(returnVal['src'], returnVal['alt'], returnVal['border'], returnVal['hspace'], returnVal['vspace'], returnVal['width'], returnVal['height'], returnVal['align'], returnVal['title'], returnVal['onmouseover'], returnVal['onmouseout']);
				} else
			...
			case "mceBackColor":
				...
				template['width'] += tinyMCE.getLang('lang_theme_wbnewtheme_backcolor_delta_width', 0);
				template['height'] += tinyMCE.getLang('lang_theme_wbnewtheme_backcolor_delta_height', 0);
			...
			case "mceColorPicker":
				if (user_interface) {
					...
					template['width'] += tinyMCE.getLang('lang_theme_wbnewtheme_colorpicker_delta_width', 0);
					template['height'] += tinyMCE.getLang('lang_theme_wbnewtheme_colorpicker_delta_height', 0);
			...
			case "mceCodeEditor":
				...
				template['width'] = parseInt(tinyMCE.getParam("theme_wbnewtheme_source_editor_width", 720));
				template['height'] = parseInt(tinyMCE.getParam("theme_wbnewtheme_source_editor_height", 580));
			...
			case "mceCharMap":
				...
				template['width'] += tinyMCE.getLang('lang_theme_wbnewtheme_charmap_delta_width', 0);
				template['height'] += tinyMCE.getLang('lang_theme_wbnewtheme_charmap_delta_height', 0);
			...
			case "mceInsertAnchor":
				...
				template['width'] += tinyMCE.getLang('lang_theme_wbnewtheme_anchor_delta_width', 0);
				template['height'] += tinyMCE.getLang('lang_theme_wbnewtheme_anchor_delta_height', 0);
	...
	getEditorTemplate : function(settings, editorId) {
		...
		var template = new Array();
		var deltaHeight = 0;
		var resizing = tinyMCE.getParam("theme_wbnewtheme_resizing", false);
		var path = tinyMCE.getParam("theme_wbnewtheme_path", true);
		var statusbarHTML = '<div id="{$editor_id}_path" class="mceStatusbarPathText" style="display: ' + (path ? "block" : "none") + '">&#160;</div><div id="{$editor_id}_resize" class="mceStatusbarResize" style="display: ' + (resizing ? "block" : "none") + '" onmousedown="tinyMCE.themes.wbnewtheme._setResizing(event,\'{$editor_id}\',true);"></div><br style="clear: both" />';
		var layoutManager = tinyMCE.getParam("theme_wbnewtheme_layout_manager", "SimpleLayout");

		// Setup style select options -- MOVED UP FOR EXTERNAL TOOLBAR COMPATABILITY!
		var styleSelectHTML = '<option value="">{$lang_theme_style_select}</option>';
		if (settings['theme_wbnewtheme_styles']) {
			var stylesAr = settings['theme_wbnewtheme_styles'].split(';');
			...
			TinyMCE_WbnewthemeTheme._autoImportCSSClasses = false;
		}

		switch(layoutManager) {
			case "SimpleLayout" : //the default TinyMCE Layout (for backwards compatibility)...
				var toolbarHTML = "";
				var toolbarLocation = tinyMCE.getParam("theme_wbnewtheme_toolbar_location", "bottom");
				var toolbarAlign = tinyMCE.getParam("theme_wbnewtheme_toolbar_align", "center");
				var pathLocation = tinyMCE.getParam("theme_wbnewtheme_path_location", "none"); // Compatiblity
				var statusbarLocation = tinyMCE.getParam("theme_wbnewtheme_statusbar_location", pathLocation);
				var defVals = {
					theme_wbnewtheme_buttons1 : "bold,italic,underline,strikethrough,separator,justifyleft,justifycenter,justifyright,justifyfull,separator,styleselect,formatselect",
					theme_wbnewtheme_buttons2 : "bullist,numlist,separator,outdent,indent,separator,undo,redo,separator,link,unlink,tooltip,anchor,image,cleanup,help,code",
					theme_wbnewtheme_buttons3 : "hr,removeformat,visualaid,separator,sub,sup,separator,charmap"
				};
				...
				// Render rows
				for (var i=1; i<100; i++) {
					var def = defVals["theme_wbnewtheme_buttons" + i];

					var buttons = tinyMCE.getParam("theme_wbnewtheme_buttons" + i, def == null ? '' : def, true, ',');
					if (buttons.length == 0)
						break;

					buttons = removeFromArray(buttons, tinyMCE.getParam("theme_wbnewtheme_disable", "", true, ','));
					buttons = addToArray(buttons, tinyMCE.getParam("theme_wbnewtheme_buttons" + i + "_add", "", true, ','));
					buttons = addToArray(tinyMCE.getParam("theme_wbnewtheme_buttons" + i + "_add_before", "", true, ','), buttons);
			...
			case "RowLayout" : //Container Layout - containers defined in "theme_wbnewtheme_containers" are rendered from top to bottom.
				template['html'] = '<table class="mceEditor" border="0" cellpadding="0" cellspacing="0" width="{$width}" height="{$height}" style="width:{$width}px;height:{$height}px"><tbody>';

				var containers = tinyMCE.getParam("theme_wbnewtheme_containers", "", true, ",");
				var defaultContainerCSS = tinyMCE.getParam("theme_wbnewtheme_containers_default_class", "container");
				var defaultContainerAlign = tinyMCE.getParam("theme_wbnewtheme_containers_default_align", "center");
				...
				for (var i = 0; i < containers.length; i++)
				{
					...
					else if (containers[i] == "mceElementpath" || containers[i] == "mceStatusbar") // ... mceElementpath:
					{
						...
						var curContainer = tinyMCE.getParam("theme_wbnewtheme_container_"+containers[i], "", true, ',');
						var curContainerHTML = "";
						var curAlign = tinyMCE.getParam("theme_wbnewtheme_container_"+containers[i]+"_align", defaultContainerAlign);
						var curCSS = tinyMCE.getParam("theme_wbnewtheme_container_"+containers[i]+"_class", defaultContainerCSS);
			...
			case "CustomLayout" : //User defined layout callback...
				var customLayout = tinyMCE.getParam("theme_wbnewtheme_custom_layout","");
	...
	initInstance : function(inst) {
		if (tinyMCE.getParam("theme_wbnewtheme_resizing", false)) {
			if (tinyMCE.getParam("theme_wbnewtheme_resizing_use_cookie", true)) {
				var w = TinyMCE_WbnewthemeTheme._getCookie("TinyMCE_" + inst.editorId + "_width");
				var h = TinyMCE_WbnewthemeTheme._getCookie("TinyMCE_" + inst.editorId + "_height");

				TinyMCE_WbnewthemeTheme._resizeTo(inst, w, h, tinyMCE.getParam("theme_wbnewtheme_resize_horizontal", true));
			}
		}
	...
	handleNodeChange : function(editor_id, node, undo_index, undo_levels, visual_aid, any_selection, setup_content) {
		...
		tinyMCE.switchClass(editor_id + '_litag', 'mceButtonDisabled');
		tinyMCE.switchClass(editor_id + '_tooltip', 'mceButtonDisabled');
		...
		if (anchorLink || any_selection) {
			...
			tinyMCE.switchClass(editor_id + '_tooltip', anchorLink ? 'mceButtonSelected' : 'mceButtonNormal');
			tinyMCE.switchClass(editor_id + '_litag', anchorLink ? 'mceButtonSelected' : 'mceButtonNormal');
		}
		...
		if (selectElm) {
			TinyMCE_WbnewthemeTheme._setupCSSClasses(editor_id);
	...
	_setupCSSClasses : function(editor_id) {
		...
		if (!TinyMCE_WbnewthemeTheme._autoImportCSSClasses)
			return;
	...
	_resizeEventHandler : function(e) {
		var resizer = TinyMCE_WbnewthemeTheme._resizer;
		...
		switch (e.type) {
			...
			case "mouseup":
				TinyMCE_WbnewthemeTheme._setResizing(e, editorId, false);
				TinyMCE_WbnewthemeTheme._resizeTo(tinyMCE.getInstanceById(editorId), resizer.width + dx, resizer.height + dy, resizer.horizontal);
				...
				if (tinyMCE.getParam("theme_wbnewtheme_resizing_use_cookie", true)) {
					...
					// Set the cookies
					TinyMCE_WbnewthemeTheme._setCookie("TinyMCE_" + editorId + "_width", "" + (resizer.horizontal ? resizer.width + dx : ""), expires);
					TinyMCE_WbnewthemeTheme._setCookie("TinyMCE_" + editorId + "_height", "" + (resizer.height + dy), expires);
				}
				break;
		}
	},
	...
	_setResizing : function(e, editor_id, state) {
		...
		if (state) {
			...
			if (!resizer.eventHandlers) {
				if (tinyMCE.isMSIE)
					tinyMCE.addEvent(document, "mousemove", TinyMCE_WbnewthemeTheme._resizeEventHandler);
				else
					tinyMCE.addEvent(window, "mousemove", TinyMCE_WbnewthemeTheme._resizeEventHandler);

				tinyMCE.addEvent(document, "mouseup", TinyMCE_WbnewthemeTheme._resizeEventHandler);
			...
			resizer.horizontal = tinyMCE.getParam("theme_wbnewtheme_resize_horizontal", true);
		} else {
	...
	_insertLink : function(href, target, title, onclick, style_class, style_rel) {
		tinyMCE.execCommand('mceBeginUndoLevel');
		...
		if (tinyMCE.selectedInstance && tinyMCE.selectedElement && tinyMCE.selectedElement.nodeName.toLowerCase() == "img") {
			...
			tinyMCE.setAttrib(linkElement, 'rel', style_rel);
		...
		if (!tinyMCE.linkElement && tinyMCE.selectedInstance) {
			...
			for (var i=0; i<elementArray.length; i++) {
				...
				tinyMCE.setAttrib(elementArray[i], 'rel', style_rel);
			}
		...
		if (tinyMCE.linkElement) {
			...
			tinyMCE.setAttrib(tinyMCE.linkElement, 'rel', style_rel);
		}

		tinyMCE.execCommand('mceEndUndoLevel');
	},
	
		_insertLitag : function(href, target, title, onclick, lilabel) {
		tinyMCE.execCommand('mceBeginUndoLevel');
		...
		if (tinyMCE.selectedInstance && tinyMCE.selectedElement && tinyMCE.selectedElement.nodeName.toLowerCase() == "img") {
			...
			tinyMCE.setAttrib(linkElement, 'title', title);
			tinyMCE.setAttrib(linkElement, 'onclick', onclick);
			tinyMCE.linkElement.innerHTML = lilabel;
...
_insertTooltip : function(text, selectedText) {

		tinyMCE.execCommand('mceBeginUndoLevel');
	
		var fullText = '<span style="visibility: hidden;" class="tooltip">' + text + "</span>" + selectedText;
		var fullText = 	'<span style="margin-top: -55px; left: 277px; visibility: hidden;" class="tooltip"><span>' + text + '</span></span>' + selectedText;
		...
		if (tinyMCE.selectedInstance && tinyMCE.selectedElement && tinyMCE.selectedElement.nodeName.toLowerCase() == "img") {
			...
			if (!linkElement) {
				...
				oldValue = linkElement.innerHTML.value;
			}
			
			fullText = fullText + oldValue;

			tinyMCE.setAttrib(linkElement, 'href', 'javascript:;');
			tinyMCE.setAttrib(linkElement, 'title', '');
			tinyMCE.setAttrib(linkElement, 'class', 'tooltiplink');
			tinyMCE.setAttrib(linkElement, 'onmouseout', 'hideTooltip(this);');
			tinyMCE.setAttrib(linkElement, 'onmouseover', 'showTooltip(this,event);');
			tinyMCE.setAttrib(linkElement, 'rel', 'nofollow');
			tinyMCE.setAttrib(linkElement.innerHTML, 'value', fullText);
		...
		if (tinyMCE.selectedInstance) {
			...
			for (var i=0; i<elementArray.length; i++) {
				...
				*/
				...
				tinyMCE.setAttrib(elementArray[i], 'href', 'javascript:;');
				tinyMCE.setAttrib(elementArray[i], 'title', '');
				tinyMCE.setAttrib(elementArray[i], 'class', 'tooltiplink');
				tinyMCE.setAttrib(elementArray[i], 'onmouseout', 'hideTooltip(this);');
				tinyMCE.setAttrib(elementArray[i], 'onmouseover', 'showTooltip(this,event);');
				tinyMCE.setAttrib(elementArray[i], 'rel', 'nofollow');
				tinyMCE.setInnerHTML(elementArray[i], fullText);
			}
...
tinyMCE.addTheme("wbnewtheme", TinyMCE_WbnewthemeTheme);

// Add default buttons maps for wbnewtheme theme and all internal plugins
tinyMCE.addButtonMap(TinyMCE_WbnewthemeTheme._buttonMap);
