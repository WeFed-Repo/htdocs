...
function setVisitedLink(aelement)
{
	...
	//Se non c'è gia' l'immagine (ovvero ci ha gia' cliccato)
...
function showCond (obj) 
{
	...
	$(obj).addClass('opened');
	if (contBox) //se esistono più box racchiusi in un id
		{
			progZindex();
		}
}  
...
// dim: può avere come valore 'large', 'medium', 'small' oppure un numero per la larghezza in pixel
...
function showCondOverlayer(obj,elToAnchor) 
{
	...
	elToAnchor = ($(obj).closest('.ancoraT').length) ? $(obj).closest('.ancoraT') : $(obj); // altrimenti è l'obj stesso
