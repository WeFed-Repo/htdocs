	...
	if (the_width > contenuti.width())
		{
			...
			if (delta > 4)
			{
				$('#menufirst').css("width", 761 + delta + 'px');
				$('#menusecond').css("width", 761 + delta + 'px');
				$('#menufirst').css("backgroundImage",'url(/img/ret/str_menulevel1_bg_large.gif)');
				$('#menusecond').css("backgroundImage",'url(/img/ret/str_menulevel2_bg_large.gif)');
				closeDiv1 = $('<div>').addClass("closer");
				$('#menufirst').append(closeDiv1);
				closeDiv2 = $('<div>').addClass("closer");
				$('#menusecond').append(closeDiv2);
			}
		}
	newBr = $('<br>').addClass("clear");
	var newBr = ifr.parent().append(newBr);
