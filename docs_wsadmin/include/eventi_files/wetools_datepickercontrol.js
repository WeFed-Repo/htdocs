...
DatePickerControl.todayText       = "Oggi";
DatePickerControl.buttonTitle     = "";
...
DatePickerControl.htmlcode 		  = "<input type=\"text\" id=\"[idOfField]\" name=\"[nameOfField]\" size=\"10\" class=\"datepicker\" [defaultValue]/>"; //Codice HTML del bottone
DatePickerControl.initialized 	  = false; //Verifica che non sia gia' inizializzato


DatePickerControl.Months = ["Gennaio", "Febbraio", "Marzo", "Aprile", "Maggio", "Giugno", "Luglio", "Agosto", "Settembre", "Ottobre", "Novembre", "Dicembre"];

DatePickerControl.Days = ["Do", "Lu", "Ma", "Me", "Gi", "Ve", "Sa"];
...
DatePickerControl.editIdPrefix    = "WtDate_";       // The prefix for edit's id
DatePickerControl.editIdClassName = "datepicker";    // Classe identificativa del datepicker
...
{
	// Crea l'oggetto datepicker, se necessario
	if (!document.getElementById("CalendarPickerControl"))
		...
		if (this.useTrickyBG){
			...
			this.calBG.style.border   = "solid 0px #dedede";
	...
	for (i=0; i<inputsLength; i++){
		if (inputControls[i].type.toLowerCase() == "text"){
			...
			// if fomated id attr:
			else if (editctrl.id && editctrl.className.indexOf(this.editIdClassName) == 0){
...
DatePickerControl.createButton = function(input, useId)
{
	...
	calButton.src = "/img/btn_pickdate.gif";
	...
	noBreak.className = "datepickernobr";
...
{
	var vCode = "";
	vCode += "<table class='calframe' id='" + this.calFrameId + "' cellpadding='0' cellspacing='0'>";
...
DatePickerControl.getHeaderCode = function()
{
	...
	vCode += "onclick='DatePickerControl.build(" + this.month + ", " + (parseInt(this.year)-1)+");return false;'><a href='javascript:;'>&laquo;</a></span>";
	vCode += "&nbsp;" + this.year + "&nbsp;";

	vCode += "<span title='" + this.Months[this.month] + " " + (parseInt(this.year)+1) + "' class='yearbutton' ";
	vCode += "onclick='DatePickerControl.build(" + this.month + ", " + (parseInt(this.year)+1) + ");return false;'><a href='javascript:;'>&raquo;</a></span>";
	vCode += "</td></tr>";
	...
	vCode += "onclick='DatePickerControl.build(" + prevMM + ", " + prevYYYY + ");return false;'><a href='javascript:;'>&lt;&lt;</a></td>";
	...
	vCode += "<a href='javascript:;'>" + this.todayText + "</a></td>";

	vCode += "<td class='navbutton' title='" + this.Months[nextMM] + " " + nextYYYY + "' ";
	vCode += "onclick='DatePickerControl.build(" + nextMM + ", " + nextYYYY +	");return false;'><a href='javascript:;'>&gt;&gt;</a></td>";
	...
	for (j=vFirstDay-this.firstWeekDay; j<7; j++) {
		if (this.isInRange(vDay)){
			...
			vCode += "<td class='" + classname + "' class_orig='" + classname + "' " +
				"onClick='DatePickerControl.writeDate(" + vDay + ")' id='" + this.dayIdPrefix + vDay + "'><a href='javascript:;'>" + vDay + "</a></td>";
		}
		else{
...
/*  Creazione del campo datepicker in modo "verticale" */
DatePickerControl.createDp = function (idOfField, defaultValue)
{
	//crea il campo di testo piu' il bottone
	var HTMLToWrite = DatePickerControl.htmlcode.replace("[idOfField]",idOfField);
	HTMLToWrite = HTMLToWrite.replace("[nameOfField]",idOfField);
	if (defaultValue)
		{
			HTMLToWrite = HTMLToWrite.replace("[defaultValue]","value = \"" + defaultValue + "\"");
		}
	else
		{
			HTMLToWrite = HTMLToWrite.replace("[defaultValue]","");
		}
	document.write(HTMLToWrite);
	
	//aggiunge, onload, quanto necessario al DP per funzionare
	if (!DatePickerControl.initialized)
	{
		if (window.addEventListener){
			window.addEventListener("load", DPC_autoInit, false);
		}
		...
		DatePickerControl.initialized = true;
	}
}