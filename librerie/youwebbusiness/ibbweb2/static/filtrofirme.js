...
var ESITI_SEDA 				= "3345";   //it.itecons.actions.iface.ActionFunctionIds.ESITI_INCASSI_SEPA = "3345"
var FIRMA_DISTINTE 			= "2800";   //it.itecons.actions.iface.ActionFunctionIds.ESITI_INCASSI_SEPA = "3345"
...
function detailIntestatario(){
	...
	switch (id_Func){
	case FIRMA_DISTINTE:
		detailIntestatario_Firme(intestatario);
		break;
	}
}

function detailIntestatario_Firme(intestatario) {
...
function showParameters(){
	...
	case FIRMA_DISTINTE:
		...
		showParameters_Firma_Distinte(flag);
		break;		
	}
...
function showParameters_Firma_Distinte(flag) {
...
function reopenParameters() {
	...
	switch (id_Func){
		...
		case ESITI_INCASSI:
			showParameters_Esiti_Pag(flag);
			break;
		case FIRMA_DISTINTE:	//it.itecons.actions.iface.ActionFunctionIds.ESITI_PAGAMENTI = "2310"
			showParameters_Firma_Distinte(flag);
			reopenParameters_Firma_Distinte(flag);
			break;			
	}
...
function reopenParameters_Esiti_Pag(flag){
	var field = document.forms[0].elements['filtroFirma.tipoEsito'];
	if (field.style.removeProperty) {
		field.style.removeProperty ("border-color");
	}else{
		field.style.removeAttribute ("borderColor");
	}
	field = document.forms[0].elements['filtroFirma.statoPagamento'];
	if (field.style.removeProperty) {
		field.style.removeProperty ("border-color");
	}else{
		field.style.removeAttribute ("borderColor");
...
function reopenParameters_Firma_Distinte(flag) {
	var field = document.forms[0].elements['filtroFirma.statoPagamento'];
	if (!field) {
		return;
	}
	if (field.style.removeProperty) {
...
function setReadOnlyField() {
	/*
	document.forms[0].elements['filtroFirma.ricercaDistinta.dataCreazioneA'].value="";
	document.forms[0].elements['filtroFirma.ricercaDistinta.dataCreazioneDa'].value="";
	document.forms[0].elements['filtroFirma.ricercaDistinta.importoA'].value = "";
	document.forms[0].elements['filtroFirma.ricercaDistinta.importoDa'].value = "";
		*/
	
	/*
	lockFieldByName('filtroFirma.ricercaDistinta.dataCreazioneA', true, true);
	lockFieldByName('filtroFirma.ricercaDistinta.dataCreazioneDa', true, true);
	lockFieldByName('filtroFirma.ricercaDistinta.importoA', true, true);
	lockFieldByName('filtroFirma.ricercaDistinta.importoDa', true, true);
	*/

	if (document.forms[0].elements['filtroFirma.statoPagamento'] !== undefined){
		document.forms[0].elements['filtroFirma.statoPagamento'].disabled=false;
	 	document.forms[0].elements['filtroFirma.statoPagamento'].style.backgroundColor = "#ffffff";
	}
}
...
if(this.ckecked) { this.form.elements['filtroFirma.ricercaPerSottoDistintaAttivo'].checked = false; }
