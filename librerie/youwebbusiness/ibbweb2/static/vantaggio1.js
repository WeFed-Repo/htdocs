...
/*1.3.1.6*/			myWin = window.open(indirizzo, titolo, parametri);
//produzione		myWin = window.open("../../ibbweb/"+indirizzo, titolo, parametri);
				...
				function verificaSportello()
				{
					...
					if(abiDes!='' && cabDes!='')
					{
/*1.3.1.6*/				threeParametersServlet('../../ibbweb/verifyabicab','cod-abi',abiDes,'cod-cab',cabDes,'verify','true','height=280,width=350,resizable=no,scrollbars=yes,screenX=600,dependent=yes','Selezionare tutti i campi obbligatori.');
...
/*1.3.1.6*/					gotoservlet("../../ibbweb/lookupagency","CercaAgenzia");
						}
						else
						{
...
/*1.3.1.6*/					gotoservlet("../../ibbweb/lookupagency?cod-abi="+document.forms[0].elements['anagrafica.coordinateBanca.ABI'].value+"&command=Cerca","CercaAgenzia");
...
function waitingMessagePdf(windowOpened)
{
	...
	{	
		...
		// problema relativo alla stampa di più pdf senza aver prima
...
function collaps(id){				
		...
		if (field.style.display && field.style.display =='none')
		{
			...
			document.all(nomeImg).src = "../static/images/asc.gif";
		}
		else
		{
			field.style.display = 'none';
			document.all(nomeImg).src = "../static/images/desc.gif";
		}
...
/*
	Funzione che setta la visibilità dei campi all'occorenza
*/
...
/*
	...
	nei bonifici sepa e in questa personalizzazione vi è il controllo
