...
function submit(action) {
    if (document.masterform['pagesize'].value!='') action = action+ "&checkIncassiDisposizioniPageSize="+document.masterform['pagesize'].value;
...
function getFirstItemForPage(requestedPage)
{
        ...
        var esitiPageSize = (stringIsEmpty(document.masterform['pagesize'].value)) ? getQueryParam('checkIncassiDisposizioniPageSize') : document.masterform['pagesize'].value;
...
function navigaPerPagina() {
    ...
    var offsetParam = 'checkIncassiDisposizioniOffset';
    ...
    var url = location.protocol + "//" + location.host + '/ibbweb2/incassi/ricCheckSottodistinta.action'; //location.pathname;
    ...
    function goToPage(valorePagina) {
        ...
        var eoElement = document.forms[0].elements['checkIncassiDisposizioniOffset'];
        ...
        for(var i=0; i < keyValuePairs.length ; i++) {
            ...
            if (keyValuePairs[i].indexOf('checkIncassiDisposizioniOffset') === 0) {
                _offset = true;
            }
        ...
        if (!_offset) {
            tmp += 'checkIncassiDisposizioniOffset='+first_item+'&';
        }
        if (!_dtSorted) {
            tmp += 'fromFilter=true&';
        }
