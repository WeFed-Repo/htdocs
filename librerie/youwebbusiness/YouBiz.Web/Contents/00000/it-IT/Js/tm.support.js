...
//TODO inserito David per stamapre ammount richiesto da Benaglio in ListSaldoRapportoAndAziendaKnockout
function FormatAmmountDecimal(decimalValue) {
    if (decimalValue) {
        return decimalValue.formatMoney(2, ',', '.');
    }
    return '';
}
...
//TODO inserito David per stamapre ammount come richiesto da Benaglio in ListSaldoRapportoAndAziendaKnockout
...
function navigateFormForExport(url,parameter) {
    navigateFormByParam({
            methodMenu: 'post',
            target: '_new',//Attenzione verificare target con browser diversi
            urlmenu: url,
...
function getSafeResource(resourceName) {
