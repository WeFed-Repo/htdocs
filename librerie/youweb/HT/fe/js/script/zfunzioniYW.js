...
$(document).ready(function(e) {
	...
	$.datepicker.setDefaults({ 
		showOn: 'both', 
		buttonImage: getPathImages()+'nGrafica/calendar.png', 
...
function popolaSelezioneCC(contoSelezionato, ctx) {
	...
	if (typeof contoSelezionato.saldoDisponibile != 'undefined')
	{
		...
		//Se il valore termina per un numero (non c'è già la divisa agganciata) allora aggiungo la divisa
	...
	$("#tooltip_popover > div").each(function(){
		if ($(this).attr("data-auto")=="false") {
			...
			if (element!=undefined) {
				element.hover(function(){parsePopover("help_"+$(this).attr('id'));}, function(){$("#"+$(this).attr('id')).popover('destroy');});
			}
		}
	})
});
...
function OpenHelp(page,modalHeader) {
	$.ajax({
  		url: page,
 		success:function(data){
 			$('#helpDialog .modal-header').html('<h1><span>' + modalHeader + '</span><a class="close btn-icon" data-dismiss="modal" aria-label="Close"><i class="icon icon-alert_error_fill icon-2x"></i></a></h1>');
...
(function ($) {
	...
	$.fn.cardBoxTable = function(options) {
        ...
        var settings = $.extend({
        	...
        	defaultValue: [], 			// lista dei valori di default se il valore della colonna è vuoto
        	show: function() {},		// callback che viene fatta la call dopo che cardbox è stato creato
        								// $(this) è il nuovo div creato per il cardbox layout
...
/* INIZIALIZZAZIONI CONDIVISE */
$(function(){

	/* Selettore rapporti (base) */
	$(".selector .selector-els .selector-el").on("click",function(){
		...
		srwrap.find("input").val(srel.attr("data-idel")).trigger("change");
		// Html del bottone
