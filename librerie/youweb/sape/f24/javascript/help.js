...
function helpIMU(str) {
   window.open(str,'help','toolbar=no,location=no,status=no,scrollbars=yes,resizable=yes,width=700,height=550,left=10,top=10');
}

function helpSemplificato(str) {
   window.open(str,'help','toolbar=no,location=no,status=no,scrollbars=yes,resizable=yes,width=915,height=644,left=10,top=10');
}
...
function helpRubrica(str) {
  ...
  <!-- inizio sezione relativa alla valorizzazione automatica dei campi codici tributi -->
	...
	function ricercaTributoSemplificato(oggetto, tipoRicerca, sezione, showHelpAction) {
		if (lastDeselected) {
			lastDeselected.focus();
		}
		...
		help(showHelpAction + '?tipoRicerca=' + tipoRicerca + '&returnField=' + returnField + '&fieldValue=' + fieldValue + '&sezione=' + sezione);
	}
